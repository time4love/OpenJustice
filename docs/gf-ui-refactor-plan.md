...
pages of UI-5 and UI-6, every backend file — **LIFTED 2026-09-20 (the researcher, R66 „Q1 transcript approved · Q2 add fields”) for exactly: the transcript builders and `thesisPredicates.history` (thesis §9 :974, A4 :1476), `getThesisContext.ts`, `getDebate.ts` / `debateState.ts` (the record named, `turns` from the builder), `getFraming.ts` (its rounds through the builder), `listPages.ts` (`trackedUrlId`, `public`, `stopPending`, interaction A5 :1071), `listTheses.ts` (the state through `publicationState`), and their tests; the surface stays 46; no migration.** **WIDENED 2026-09-22 (the researcher, R71 „approve Q1”): the lift ALSO names `services/publishedThesis.ts`, `services/criticMaterial.ts`, `services/corpusReads.ts` and `services/thesisReviews.ts`.** The ground is the read's cost (`docs/gf-thesis-read-cost-2026-09-22.md`): PR #560 already edited the first three (`publishedThesis.ts` +155/−52, `criticMaterial.ts` +22/−10, `corpusReads.ts` +4/−2) and THIS LINE IS WHAT SHOULD HAVE MOVED WITH THEM — `:709`'s widening named only `getThesisContext.ts`, so three backend KEEP files were edited with no clause naming them, found by a cold read at R71. `criticMaterial.ts` is why the set cannot be smaller: it holds `headFingerprint` :98, the ONE loader `run_analysis` refuses on (`criticMaterial.ts` :33–:34), so the read and the refusal cannot share rows unless it is inside the lift. `thesisReviews.ts` joins for its own corpus walk per entry (:173–:174). **RULED 2026-09-22 (the researcher, R71): the lift ALSO permits the NEW module `services/thesisRows.ts`** — the one loader of the thesis read, holding the only `prisma` for it, over which `transcriptOf`, `headFrom`, `fingerprintOf` and `citationsFrom` become pure functions of rows. It is named because KEEP here reads *every backend file* and a sweep counts an ADDITION as a row, so an unnamed new module would go red every round; and because UI-8 owning chunk A while forbidding the module chunk A is defined as would be a step that cannot be executed. **It shares ROWS and never a verdict** — thesis A3 :1413–:1414, *„every predicate is computed on read and none is stored … this document adds no cache”*, which `corpusReads.ts` :522–:525 already states of itself. **No OTHER new backend file is lifted by this clause**; the surface stays 46; no migration.
...
**STATUS:** **CLOSED 2026-09-22 (the REDUCED set) — `docs/gf-ui-step-8-2026-09-22.md`.** Fourteen landings (#544, #545, #546, #548, #549, #553, #558, #560, #561, #562, #563, #564, #565, #566; `staging` `bca6d2b`): the transcript, the reader with its types, parsers and fixture set, `/research`'s four regions, the corpus facet's SHAPE, the gated corpus doors, the page a view is about, the working view's read and context block, the loader rebuilt in two chunks, the owed row riding the read, the pane with its declared tab and door, the fold-in, and THE CENTRE — the tick line and the text, UI-5's `components/thesis/*` CALLED (plan :739 (i)–(ii), board ד2). Recorded there: the loader's cost and what it cost to find (`docs/gf-thesis-read-cost-2026-09-22.md`, 39 delegate calls, the body assembled three times, a ~550 ms per-query floor, 813–835 ms deployed from ONE request); the HISTORY SIZE, **now TAKEN** — 37 turns, 6,768 px, **9.08 screens at 375 px**, 518 nodes, identical local and staging, so NO cursor is added (§41 :1093); the CENTRE measured at 375 px (1,472 px, the page 3.23 screens, no horizontal scroll, no sticky element, and NO legal disclaimer rendered — `COMPLIANCE.md` :92 holds); **NO LIBRARY was chosen and the step records choosing none**, its dependency delta zero; the exercise on the real staging body with ITS RAW SOURCE KEPT BESIDE IT (`handoffs/R73-page-reading-raw-2026-09-22.txt`, the protocol's rule of 2026-09-22); the disclaimer FALSE ALARM — a probe over `document.body.textContent` read the sentence out of Next.js's RSC payload inside a `<script>` while the landed instrument, which reads through `shownText()`, was right all along; and the PARSER that had narrowed a citation to four fields since R70, dropping `record`, `content`, `verified`, `flag` and `overObjection` with nothing able to fail on it. **RULED 2026-09-22 (the researcher, R71): THE LOADER REBUILD IS THIS STEP'S WORK and its record closes HERE, inside UI-8 — not beside #547**, which is a SEPARATE defect and is untouched. **DEFERRED, each an issue carrying its design reference, not lost:** the five remaining pane tabs (CITATIONS · GAPS · ANALYSIS · FRAMING · APPEALS, §14 :486's end state — the centre's RECORD tabs are §18's and are NOT among them); the framing sheet, board ח (`FramingRow.tsx` :14–:19 already declares it owed); and the PUBLISHED toggle and the version diff (:739 (iii)), with `diff-is-of-two-texts` deferring alongside them. **OBSERVED AND OPEN, the researcher's at the page:** the tick line WRAPS 1 + 2 at 375 px with no horizontal scroll (ד2 is the 1440 board), and the live tick ORDER is chronological while ד2 draws 5.8 · 28.6 · 6.9 because `boards.py` :330 builds from `DEBATE_OPENED` turns — the board is the artefact, not the page. **RE-BRIEFED 2026-09-16** against the design canvas, pages 1 and 5 — docs/gf-ui-design-session-2026-09-16.md §4.
