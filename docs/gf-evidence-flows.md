...
list_findings({ url })                                                    PUBLIC · ⚠️ to build
  returns   { page: { url, public: bool },
              captures: [{ capture, snapshotDate, textHash, textExtractionVersion,   ← **THE ROW IS UNCHANGED; A CAPTURE'S TEXT GETS ITS OWN READ. Ruled 2026-09-19 on a cold design review the researcher commissioned and accepted whole, and it REPLACES this seat's first ruling, which had put an optional `capture` argument on THIS tool.** The new read is **`get_capture({ url, capture, textHash? })` — PUBLIC, ⚠️ to build — answering ONE capture: the row this clause already returns for it, PLUS its `text`; refusing NOT_SURVEYED · NOT_A_CAPTURE · NOT_PUBLIC; routed at `GET /api/pages/:trackedUrlId/captures/:capture`, the sibling of the mounted …/captures/:capture/chain, which is the precedent the shape follows.** WHY A RESOURCE AND NOT AN ARGUMENT: a list tool with a single-item mode makes its RETURN TYPE depend on its argument (one row in a `captures` array whose `counts` then lie), runs the whole page's timeline to answer one row — `findingsOf` loads every capture, every diff, attribution and linkage — and makes `list_findings` a conditional superset of `list_corpus` against `listCorpus.ts`'s own parity clause; and as an MCP tool it is a list that sometimes returns a whole document into a model's context, keyed on an optional argument. **THE MCP SURFACE THEREFORE MOVES 45 → 46**, which is larger than the "one field" first described, and is written here so the growth is DECLARED and not discovered. **`textHash` IS THE CLAUSE THAT MATTERS LATER:** a citation's text is the text at its PIN (`heldTextsFor` reads a superseded `textVersion`) while a capture read by `(page, capture)` is the CURRENT extraction, so after a re-extraction the thesis pane and the corpus sheet correctly show different bytes for what a reader calls one record — and without `textHash` the thesis page could never move to reading on open without changing what it shows.** **RULED 2026-09-19 (the researcher), on the builder's question: a `textHash` naming an extraction this capture does NOT hold is refused `NOT_A_CAPTURE`, in that code's malformed-argument voice, and is NEVER answered with the current text — bytes other than the ones asked for, presented as the ones asked for, are the fabrication class this platform refuses; the three codes stay three, and no fourth is minted.**
                           anchor: { documentHash, attributed: bool } }],
              diffs:    [{ before, after, current: { contentVersionHash, chunks } | null,
                           awaitingDerivation: bool,
                           opinion: { significance, categories, legallySignificant: bool, editorial: BOOL,
                                      classifierVersion, draws: NUMBER } | null   — LABELLED as opinion. **TYPED 2026-09-19: `editorial` is a BOOLEAN and `draws` a NUMBER. This clause named both and typed neither, so UI-7 chunk 1 hand-wrote them `string | null` and the fixture was written from the same reading — 273 green cases said nothing, and the page 500’d on the first real body, the parser naming its own field. Two independent sources: the running backend answers `editorial: true` and `draws: 1`, and ui flows §24 :673 writes the same shape — "20 carry `editorial: true`". Nullability is UNVERIFIED: the observed row had values for both, so they are typed required. `draws`’ MEANING is undocumented and nothing renders it.**,
                           narrowed: bool,
...
get_diff_input({ url, before, after })                                    PUBLIC
  does      the pair's two current texts and the CURRENT version's chunks — named by the pair, **THE ENVELOPE, WRITTEN 2026-09-19 (R64) because this clause named the FIELDS and not the SHAPE, and UI-7's frontend wrote `before`/`after` as STRINGS with `beforeText`/`afterText` beside them and an `awaitingDerivation` flag — a shape nothing serves; read from `getDiffInput.ts`' `interface DiffInput` and one body on the running route (the :1112 precedent):** `{ page: { url, public }, before: { capture, textHash, textExtractionVersion, text }, after: { capture, textHash, textExtractionVersion, text }, current: { contentVersionHash, diffVersion, chunks: [{ side: 'REMOVED' | 'ADDED', text, survival }] }, opinion: (as :1086–:1087) | null, narrowed: bool, evidence: (as :1089–:1090) | null }` — **RULED 2026-09-20 (the researcher): THE DIFF ROW PLUS THE TWO TEXTS, as `get_capture` is the capture row plus its text (:1082) — `opinion`, `narrowed` and `evidence` are the SAME fields `list_findings`' diff row carries, built by the same builders and never re-spelled, so the diff page can draw the opinion labelled, the NARROWED mark and the citing theses that ui §26 :852–:856 asks of it; the intervening captures behind `narrowed` are served by no read and stay owed** — each SIDE is an OBJECT carrying its own bytes; `current` is NEVER null in a 200 and carries no flag: an undefined CURRENT is the AWAITING_DERIVATION refusal, `{ error, code }` at 409 (ui §6 :267), and the four "not found" codes are the public door's ONE 404 body with the code hidden (ui §6 :266); `chunks` is the stored version's column AS STORED, so a reader narrows `side` to the union at its own boundary.**
...
verify_claim_text({ url, phrase })                                         GATED — amended by docs/gf-ui-flows.md §6 (2026-09-15, #487): each call reaches the Internet Archive, unbounded work against a free service; one capture's raw-archive check, for a researcher
  does      the corpus read by text against the raw archive and the stored text; the public search is search_corpus (the stored register) — **2026-09-24 (the researcher, R81 Q3): a document ARM, `{ commitment, phrase }`, is document flows A4 :1470's**
...
resolve_record({ fileHash })                                               PUBLIC · ⚠️ to build
  does      what a stranger holding a citation needs: the record the name resolves to — kind, **THE ENVELOPE, WRITTEN 2026-09-19 (R64) for the reason :1112 and :1096 state — UI-7's frontend guessed `first`/`last` dates, a boolean `verified`, a flat `captures[]` and a boolean `flagged` beside a `publishedAt`, none of which the route sends; read from `resolveRecord.ts`' `interface Resolved` and one body on the running route:** `{ fileHash, kind: 'CAPTURE' | 'DIFF', page: { trackedUrlId, url, public }, record: { capture } | { before, after }, recomputable: bool, verified: { verified: bool, captures: [{ capture, documentHash, anchoredHash: string | null, anchoredHashMatchesDocumentHash: bool, attributed: bool | null, verdict: string | null, verifierVersion: string | null, checkedAt: ISO | null }] } | { notEvaluable: 'NOT_PROMOTED' | 'MALFORMED_RECORD_KEY' }, citedBy: [{ thesisId, versionId, contentHash, pin: string | null, flagged: { flagged: bool, armsEvaluated: [string], reasons: [string] }, text }] }` — **CONFORMED 2026-09-26 (the researcher, R84 Q12): the third reason `DOCUMENT_CLASS_NOT_BUILT` is RETIRED from `notEvaluable` — no DOCUMENT row reaches this read (a name resolves through the corpus's captures and pairs), the publication gate grades a document through its own arm since document step 34, and `verified`'s DOCUMENT arm is a loud guard** — the TIMESTAMPS this clause names are `record`'s endpoint captures (14-digit archive names, never dates); `verified` is an OBJECT or the reason it cannot be asked, shown as a reason and never a failure (ui §18 :574); `flagged` is FLAGGED's REPORT (A3 :1054), never a bit; `text` is the citing VERSION's whole text (thesis A2 :1273); a refusal is the public door's ONE 404 body for NOT_A_RECORD · NOT_PUBLIC (ui §6 :266); **`page.trackedUrlId` RULED 2026-09-20 (the researcher): a stranger arrives by the record's NAME and holds no page id, so `/records/[fileHash]`'s one link onward to the record's page (ui §26 :860) has no other source — the UI-3 precedent that gave the thesis body's `pages` the same id for the same link (#488); `get_capture` and `get_diff_input` keep `{ url, public }` because their reader named the page in the URL.**
...
check_on_chain_status({ url, capture })                                    PUBLIC · re-scoped
  does      a check on a CAPTURE: isRegistered · ATTRIBUTED · anchoredHash = documentHash · **THE ENVELOPE, WRITTEN 2026-09-19 because this clause named the FIELDS and not the SHAPE, and UI-7's frontend guessed a flat one that nothing serves:** `{ page: { url, public }, captures: [{ capture, documentHash, isRegistered, registryIndex: number | null, submitter: string | null, attributed, anchoredHash: string | null, anchoredHashMatchesDocumentHash, storedVerdict: { verdict, verifierVersion, checkedAt, attributed: bool | null } | null }], registry: { chainId: number | null, registryAddress: string | null } }` — ALWAYS an array of captures, one for a capture named by its page, every capture beneath a record named by its fileHash; `registry` is OBSERVED, never configured; a refusal is `{ error, code }` at 503 for CHAIN_UNAVAILABLE.**
...
open_debate({ thesisId, record, rationale })                              WRITE · ⚠️ re-shaped
  does      computes fileHash = ID(record); REFUSES unless the thesis's HEAD version mentions it;
            ...
            → ASSESSOR with the argument, the record's CURRENT computed content, and the passage
              of the head version that cites it; records RATIONALE_SUBMITTED · ASSESSMENT_RETURNED — **RULED 2026-09-25 (the researcher, R81 QA): the assessor also NAMES each assertion it makes, a span of the citing passage and the phrase it attributes to the record, and the audit records `quoteVerified` and `phraseVerified` beside each, as the framing round does (thesis T1 :256–:261, A2 :1307–:1308); the debate's audit is the verdict rule's fourth caller (document A7 :1581)**
  returns   the debate state: { sessionId, fileHash, hasSubstance, verdict, canPromote,
            blockedBy, events } — **RULED 2026-09-20 (the researcher, R66 „Q2 amend”): ONE `DebateState` for every debate tool, the shape get_debate answers at :1144 — `{ sessionId, thesisId, fileHash, record, status, hasSubstance, verdict, canPromote, blockedBy, promotedOverObjection, evidenceFileHash, turns: T[] }` from the one projection and the one builder; `events` as raw rows is RETIRED from every debate tool's answer (open_debate · respond_in_debate · get_debate; promote_from_debate answers its own shape at :1132 and never carried events — corrected 2026-09-20, R66 step 14)** — **RULED 2026-09-25 (the researcher, R81 QB): `record` has a third form, `{ commitment, title }` (document A2 :1339–:1341's `recordCommitment`; the title is A2 :1271's), ONE shape wherever a record is answered**
...
get_debate({ sessionId })                                                 GATED read · unchanged — **THE ENVELOPE, RULED 2026-09-20 (the researcher, R66 „Q1 transcript approved”; read from `debateState.ts` `DebateState` :47 and the ruling, a real body OWED): `{ sessionId, thesisId, fileHash, record: { url, capture } | { url, before, after }, status, hasSubstance: bool, verdict: string | null, canPromote: bool, blockedBy: BlockerCode[], promotedOverObjection: bool, evidenceFileHash: string | null, turns: T[] }` — `record` NAMED as A1 names it (the loader reads it; `projectDebate` :140 had dropped it), and `turns` the thread's turns of thesis A4 :1476 (DEBATE_OPENED · RATIONALE · ASSESSMENT · RESPONSE · DEBATE_CLOSED) from the SAME builder the transcript uses, oldest first — `events` as raw `{ type, content: string, at }` is RETIRED from this read with the builder; refuses SESSION_NOT_FOUND as `{ error, code }` at 404 inside the prefix; NO_RESEARCHER a wiring DEFECT there** — **RULED 2026-09-25 (the researcher, R81 QB): `record` gains the third form `{ commitment, title }`**

list_evidence_reviews({})                                                 GATED read · ⚠️ to build — **THE ENVELOPE, WRITTEN 2026-09-20 (R66) because this clause named FIELDS and not the SHAPE — the A4 :1112 / ui §6.1 :248 precedent; from the interface named and ONE real body per route, read 2026-09-20 on staging through the researcher’s own signed-in session in the pane (the local sign-in cannot complete: the auth redirect names staging): read from `evidenceReviews.ts` `ReviewsList` :143, `ReviewEntry` :112, `NotEvaluable` :136, `Cause` :76, `Citation` :92, `NarrowingMaterial` :100 and `evidencePredicates.ts` `ContentUnit` :225, `Moved` :243:** `{ owed: number, reviews: [{ kind: 'CONTENT_MOVED', fileHash, record: { url, capture } | { url, before, after }, owedSince: ISO, decisionSequence: number, affirmed: { hash, chunks: U[] }, current: { hash, chunks: U[] }, moved: { entered: U[], left: U[] }, cause: C[], citedBy: [{ thesisId, versionId, published: bool, argument: { debateSessionId, argued: bool } | null }], narrowed: { intervening: string[], narrowerDiffs: [{ before, after, current: { contentVersionHash, chunks: U[] } | null, opinion: Opinion | null }], carried: [{ text, carriedBy: [{ before, after }] }] } | null, commands: string[] }], notEvaluable: [{ fileHash, record: … | null, reason: 'AWAITING_DERIVATION' | 'AFFIRMED_VERSION_MISSING', detail }] }` where `U = { side?: 'REMOVED' | 'ADDED' | 'ABSENT', text, survival?: 'SURVIVES' | 'CONTRADICTED' | 'UNCHECKABLE' }` and `C` is a UNION, accumulated: `{ kind: 'DECISION', capture, decisionId, decisionType, waybackTimestamp: string | null, sequence, researcherId, at: ISO } | { kind: 'EXTRACTOR', capture, from, to, at: ISO } | { kind: 'DIFF_VERSION', from, to, at: ISO } | { kind: 'UNREADABLE', capture, reason }` — so the `returns` below reads `{ owed, reviews, notEvaluable }`, not bare entries, and `cause` is an ARRAY of that union, not one object** — **RULED 2026-09-25 (the researcher, R81 QB): `record`, in `reviews` and in `notEvaluable`, gains the third form `{ commitment, title }`** — **CONFORMED 2026-09-26 (the researcher, R85 Q-D, #594): an entry whose `record` is `{ commitment, title }` carries DOCUMENT causes, and its re-derivation is the EXTRACTOR cause with no new kind — `{ kind: 'EXTRACTOR', record: { commitment, title }, from, to, at: ISO }`, `from` the extractor version of `affirmed`, `to` CURRENT_EXTRACTOR and `at` the moment of the derivation that makes CURRENT(d) current — its `DocumentContentDerivation` row (document §3 :348: the derivation pass is the writer; no decision row, no person) — CONFORMED 2026-09-26 (the researcher, R86 Q-R1: "one row per derivation, (versionId, extractorVersion, at), at null only for rows derived before the table; derivedUnder dropped in the same migration; A2 :1300 and A3 :1369 conformed"): was "`to` CURRENT(d)'s extractor version, `at` CURRENT(d)'s `derivedAt`", which name the PAST when a re-derivation re-reaches an older text (A→B→A); an entry whose `record` is a page carries the capture causes above, byte for byte as served; a reader dispatches on the entry's `record` form BEFORE `kind`, so a capture cause is never asked for `record` and a document cause never for `capture`**
