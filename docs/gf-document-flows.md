...
backend      REFUSES NO_RESEARCHER · NO_BYTES · UNSUPPORTED_TYPE · TOO_LARGE (flows A8) ·
             ...
             ← { commitment, docId, custody: HELD, content: { contentVersionHash, text | null },
                 anchored: bool, equalsCapture: { url, capture } | null, existed: bool } — **`existed` CONFORMED 2026-09-22 to A4 :1409** — **CONFORMED 2026-09-23 to A4 :1409 as ruled that day (`docs/gf-document-step-30-staging-exercise-2026-09-23.md` §5, F1 ruling 1): the answer is the RECEIPT — `content` is `{ contentVersionHash } | null`, and `text` is NOT in it; the content is `read_document`'s.**
```
...
DocumentContentVersion   append-only, §3's row
  ...
  extractor · extractorVersion · derivedAt — **RULED 2026-09-23 (the researcher): plus `derivedUnder String[]`, APPEND-ONLY — every extractor version that reproduced this exact text, appended by the derivation pass when it re-derives to content the row already holds; CURRENT(d) reads MEMBERSHIP of this list and never equality on `extractorVersion` (A3 :1368, §3 :324). And `readFailed Boolean` — the reader was SELECTED and THREW, as against having found no text. A corrupt file of an accepted type is ACCEPTED AS BYTES-ONLY like any other bytes no reader can read (§3 :284) and is NEVER refused: a refusal would turn the platform's own reader failing into a reason to hold nothing, and A5 :1493's `UNREADABLE` is a key that does not open a ciphertext — a different fact that must not share one spelling. `extractor-coverage` counts a broken PDF apart from a photograph by this field (A7 :1591).** — **CONFORMED 2026-09-26 (the researcher, R86 Q-R1: "one row per derivation, (versionId, extractorVersion, at), at null only for rows derived before the table; derivedUnder dropped in the same migration; A2 :1300 and A3 :1369 conformed"): `derivedUnder` WAS this list; it is now the `DocumentContentDerivation` rows `(versionId, extractorVersion, at)`, unique per (version, extractor), APPEND-ONLY, written by the derivation pass in the same write that creates or re-reaches the version, `at` NULL only for a membership appended before the table; the column is dropped in the same migration. `read_document`'s `provenance.derivedUnder: string[]` (A4 :1426) is served from those rows, unchanged on the wire.**
...
CURRENT(d)                HELD:   the DocumentContentVersion with a DocumentContentDerivation row for
                                  CURRENT_EXTRACTOR; none → AWAITING_DERIVATION (evidence A3's name) — **RULED 2026-09-23 (the researcher): MEMBERSHIP, never equality on `extractorVersion` (A2 :1300, §3 :324) — a re-derivation that reproduces the text APPENDS to the list and moves no row, so the identical-text case is current rather than permanently awaiting.** — **CONFORMED 2026-09-26 (the researcher, R86 Q-R1: "one row per derivation, (versionId, extractorVersion, at), at null only for rows derived before the table; derivedUnder dropped in the same migration; A2 :1300 and A3 :1369 conformed"): membership is the same predicate over the `DocumentContentDerivation` rows — the version with a row `(version, CURRENT_EXTRACTOR)`; that row's `at` is the moment the version BECAME current, which the version's own `derivedAt` is not when an older text is re-reached; was "whose `derivedUnder` CONTAINS".**
...
read_document({ commitment })                                                GATED · ⚠️ to build
  returns   HELD:   { custody, docId, bytes, versions: [{ contentVersionHash, text, provenance, — **RULED 2026-09-23 (the researcher): `read_document` NEVER PUTS BYTES IN THE MODEL'S CONTEXT; `bytes` is not a field of the answer. COMPUTED text rides it; a bytes-only document whose bytes are an IMAGE rides as an MCP image block, because §6 :735–:737 needs the model to see it, under a size cap that is an operational parameter (interaction A8's kind); every other bytes-only kind — PDF, spreadsheet, media — rides as `bytesUrl`, a short-lived signed DOWNLOAD link with its expiry.** **RULED 2026-09-23 (the researcher; `docs/gf-document-step-30-staging-exercise-2026-09-23.md` §5 F1 ruling 2, §9 Q2): TEXT LAST AND CAPPED. A version carries `contentVersionHash`, `provenance`, `opinions` and its own `textUrl` — a signed link and nothing more — and NO inline text, so an older version's text (§3 :348, :350) stays reachable without riding inline. The envelope's FINAL field is `text`: CURRENT(d)'s, capped at an operational parameter of interaction A8's kind measured against the client's result cap (a 21-page paper fits, 532-row sheets do not) — **RULED 2026-09-24 (the researcher, adopting Fable's answers with REVIEW's corrections): 80,000 CHARACTERS, measured on staging: the paper's text 77,218 characters FIT, the spreadsheet's 112,602 did NOT; both numbers recorded beside the constant, named as the CLIENT's cap, re-measured when the client changes**, with `textTruncated` and the envelope's own `textUrl` — the current version's — beside `bytesUrl`.** **RULED 2026-09-24 (the researcher, Q1 of step 30's close, adopting Fable's answer with its two additions): every `textUrl` is A5 :1504's SIGNED ARM, answering `{ url, expiresAt }` as `bytesUrl` does — the same route for the envelope's link and each version's, never a bucket object (§12 :1185) and never a second route.**
                      opinion LABELLED }], current, anchored, equalsCapture, assertions } — **SHAPE, RULED 2026-09-23 (the researcher, batch item 15, adopting Fable's two corrections with `by | null`; written by the crossed seat from the step-30 interface and a real body):** `{ custody: 'HELD', commitment, docId, mimeType, byteLength, receivedAt: ISO, assertions: { title, assertedUrl, assertedAt: YYYY-MM-DD | null, derivedFrom: { commitment, title } | null }, arrivals: [{ by: { handle, mine } | null, at: ISO }] oldest first — null for a public-door arrival (step 32), versions: [{ contentVersionHash, provenance: { extractor, extractorVersion, derivedUnder: string[], readFailed, derivedFrom: 'AT_RECEIPT' | 'HELD_BYTES', derivedAt: ISO }, opinions: [{ model, promptVersion, by: { handle, mine } | null, at: ISO, body | null }] oldest first, textUrl: { url, expiresAt: ISO } | null }], current: { contentVersionHash } | { awaiting: 'AWAITING_DERIVATION' }, bytesUrl: { url, expiresAt: ISO } | null, textUrl: { url, expiresAt: ISO } | null, textTruncated: boolean, anchored: boolean — ANCHORED(d) (A3 :1366), false by construction until step 31 and never a literal, equalsCapture: { url, capture } | null, uploadUrl, text: string | null — CURRENT(d)'s, capped, LAST }` — **CONFORMED 2026-09-23 to :1425 as ruled that day: no inline text in a version; `textUrl` per version and for the envelope; `text` the final field** — an image with no computed text, within the image cap, rides BESIDE this JSON as an MCP image block, never inside it
...
describe_document({ commitment })                                     WRITE · paid · ⚠️ to build
  does      a model reads a HELD document's bytes on the researcher's word and its reading is
            appended as the OPINION of CURRENT(d) — §3's last row; never a citation — **RULED 2026-09-23 (the researcher; `docs/gf-document-step-30-staging-exercise-2026-09-23.md` §5, F3): prompt v3 — transcription ONLY for an image or a scanned PDF, EMPTY whenever computed text exists; names ALLOWED in the description as the document prints them (the opinion is gated and never published, §7), `actors` the same, forbidden staying character, intent and guilt; `maxOutputTokens` set from the prompt's field budget. A CUT answer is refused twice over — the provider's finish reason, AND a required terminal sentinel as the body schema's LAST field, so a truncated body cannot parse; the refusal writes nothing and is not retried.**
...
review_evidence             evidence A4 — unchanged over kind DOCUMENT — **RULED 2026-09-23, in scope 2026-09-24 (the researcher, R81 Q3): `verify_claim_text({ commitment, phrase })`, evidence A4 :1101, gains a document ARM and no tool: the ONE verdict rule (A3 :1385) over CURRENT(d)'s computed text — PRESENT | ABSENT | UNCHECKED, UNCHECKED where the content is bytes (§3 :359–:365) — with the surrounding lines of a PRESENT match — returns `{ environment, commitment, contentVersionHash, phrase, verdict: PRESENT | ABSENT | UNCHECKED, reason: string | null, context: { from, lines } | null, advisory }`, as served on staging 2026-09-25 (`verifyDocumentPhrase.ts` `DocumentPhraseAnswer` :30–:42, `environment` stamped by the `verify_claim_text` registration, `mcpServer.ts` :707; RULED 2026-09-25, the researcher, R83); free, writes nothing, GATED as read_document; refuses NOT_A_DOCUMENT (handed a commitment, :1398) · AWAITING_DERIVATION · SHED · NEITHER (a call naming both a capture and a commitment, or neither, decided from the input before any row is read: thesis A4 :1521's spelling for "not exactly one target", CONFORMED 2026-09-25); a phrase in and a verdict out, never a search over the text (§12 :1190–:1191); advisory and never stored — the stored verdicts are PassageVerdict's, at publication (A2 :1313)**
```
