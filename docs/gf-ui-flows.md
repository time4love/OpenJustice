...
list_trajectories({ scope, since?, until?, page?, cursor?, limit? }) ⚠️
  returns   get_claim_trajectories' rows (A4 :1103) across pages, each with its page — **THE ENVELOPE, WRITTEN 2026-09-20 (R65) because this clause named the ROWS and not the SHAPE — the A4 :1112 / :1096 / :1106 precedent; read from `corpusReads.ts`' `TrajectoryFinding` + `EntryPage` (= `TrajectoryEntry`), `listTrajectories.ts`' `TrajectoryList`, `claimTrajectory.ts`' `ChangeSpan`, and one body on the running route (26 rows over one page, `transitions` 2–6, both `finalState` values):** `{ entries: [{ patternHash, sourceStateHash, transitions: number, firstSeen: 'YYYY-MM-DD', lastSeen: 'YYYY-MM-DD', finalState: 'PRESENT' | 'REMOVED', claimCount: number, captures: [{ snapshotDate: 'YYYY-MM-DD', waybackTimestamp, present: bool }], changes: [{ snapshotDate: 'YYYY-MM-DD', waybackTimestamp, snapshotUrl, present: bool, captures: number, days: number | null, openEnded: bool }], claims: [{ trajectoryId, claimHash, claimText }], page: { trackedUrlId, url, public: bool } }], undetected: [{ trackedUrlId, url, public }], nextCursor: string | null }` — **`captures` RULED 2026-09-20 (the researcher, on two options rendered from the real body): the STORED per-capture vector of the movement (`ClaimTrajectory.observations`, identical for every member of a group by construction), one entry per capture examined, in capture order — because `changes` is the run of STATES the claim held, one span per state from the first capture on (`changes.length` is `transitions + 1`), and each span names its FIRST capture only with a COUNT of the captures in it (`captures`), the day-bound to the capture that ended it (`days`, null when a date will not parse) and whether nothing ended it (`openEnded`); so from `changes` alone the strip of §25 :791 has one tick per span and not per capture, the sheet of §25 :809 can link only the span starts, and the diff a claim left in — the pair (last capture of span i, first capture of span i+1) — is not composable, the composed non-adjacent pair being `NO_SUCH_DIFF` whenever the span held more than one capture (the first real row holds 18; two of its three diff links were 404). Measured before the ruling: every row's span counts sum to exactly the page's 22 captures. The backend diff-0 invariant is LIFTED for exactly this field (`corpusReads.ts` and `claimTrajectory.ts`' group + their tests; the surface stays 46; `snapshotUrl` is not repeated, `lib/archiveUrl.ts` composes it)**; `undetected` NAMES every page in scope whose current state no stored pass describes, so an absence never reads as "nothing moved" — the read never computes, `get_claim_trajectories` does, gated; refusals as `list_corpus`: INVALID_RANGE `{ error, code }` at 400, NOT_SURVEYED and NOT_PUBLIC the public door's ONE 404 body (§6 :266); **the ROUTE answers 200 without `page` — `page` REQUIRED is §25 :783's rule on the PAGE `/corpus/claims`, not the read's — and at one page it equals `GET /api/pages/:trackedUrlId/trajectories` row for row** — ordered by the
...
1  THE CONTEXT LINE   sticky — **RULED 2026-09-20 (the researcher, R66 „Q9 not sticky”): NOT sticky; the first block of the page, in flow. The reason is R60's, unchanged: a sticky title bar's precondition is a SHORT NAME (docs/gf-ui-step-5-2026-09-18.md), no thesis has one (A2 :1268, the claim is the heading), and this line carries the claim. It returns the day a thesis has a name.** The CLAIM, verbatim — the claim is the heading (A2 :1268) — · the provision ·
                      ...
                      PUBLISHED ≠ HEAD (n versions since) · WITHDRAWN on <date>. In flow, not sticky (the Q9 ruling above, 2026-09-20). **THE ORDER, RULED 2026-09-21 (the researcher, R70 „I1 A”), from APPROVED BOARD ד2 — which this enumeration contradicted: ONE META LINE FIRST — the STATE word · the author's handle, `mine` marked · the publication date (`publishedAt`, A4 :1476; absent on a thesis with none) — and THEN the CLAIM as the heading, the provision, the COPY of `thesis <id>`. The list above names the ELEMENTS; this names their ORDER and wins where they differ. Why the board and not the list: the state, the author and the date are metadata ABOUT the claim, and the claim runs to hundreds of characters (R60's ruling, and the reason this block is not sticky), so nothing stands above it but its own frame. **THE BOARD IS PRIOR TO THIS DOCUMENT'S PROSE ABOUT THE SAME PAGE** — `docs/gf-two-session-protocol.md`, ruled the same day, after a seat graded this block against the enumeration and never opened ד2.**
2  WHAT IS OWED       first, stop-shaped (T6 :881–:882): this thesis's entries of REVIEWS — FLAGGED
                      ...
                      UNARGUED (the mentions), ARRIVED (later, document flows) — each with its material and **RULED 2026-09-22 (the researcher, R71 „approve B”): ON THIS VIEW the entry draws the KIND, the RECORD, `owedSince` and the ONE COMMAND — and FLAGGED's MATERIAL **and its DATE** are the CITATION SHEET's, not the strip's — **CORRECTED the same day: FLAGGED's `owedSince` is COMPUTED FROM that material (`thesisReviews.ts` :228), so a date without it costs 3–6 reads per flag, and `publishedAt` alone would be a LOWER BOUND and therefore a date that OVERSTATES how long the flag has been open.** So the strip draws, per kind: FLAGGED the kind, the RECORD and the command · UNARGUED those and `פתוח מ־<date>` · STALE_TRAJECTORY the kind, the date and the command, having no record — which is `/research`'s own shape for that arm. The RECORD is drawn on both arms that have one and is FREE, from the records `citationsFrom` already resolves. The material stays in this enumeration for `/research` (§29 region 1), whose envelope A4 :1523 carries it; the working view's envelope is A4 :1476's `E & { owedSince }`, because FLAGGED's material needs evidence-side rows the thesis read does not load and this page is not spending a query on them. Where the two disagree, THIS clause names the strip and §18 names the sheet.**
...
4  THE STREAM         HISTORY(t), OLDEST FIRST — the story of how the thesis was shaped, read top to — **RULED 2026-09-20 (the researcher, R66: „Q1 transcript approved”): THE STREAM IS THE TRANSCRIPT (thesis §9 :974, A4 :1476), grouped under one heading per THREAD (a framing · a version · a debate on <record> · an analysis · a gap · a publication attempt · a withdrawal · a note), each TURN in its speaker's register — the researcher's plain and first, the model's inside rule 3's labelled container with its model and prompt version, the platform's as a mark — COLLAPSED to one row (the kind's frozen name · the speaker · the moment · the `line` DATUM; every composed phrase of the table below — the counts, the verdict, the checks — rendered by the PAGE from `body` through frozen ICU strings, RULED 2026-09-20 (the researcher, R66 „Q1 datum”): the backend authors no sentence) and EXPANDED IN PLACE to the turn's words; the SHEET opens for the turn's MATERIAL (the record, the diff to the parent, the request's addresses), never for the words. The table below maps the eleven rows onto the seventeen turn kinds; `every-kind-renders` (§15) holds the seventeen, exhaustively.** **RULED 2026-09-21 (the researcher, R68 „אופציה ב”): THE TRANSCRIPT IS THE RIGHT PANE'S DEFAULT TAB, not the page's body — the centre of `/research/theses/[id]` is the THESIS COLUMN the public page draws (§17, UI-5's components CALLED, never edited), with the state word, the COPY and WHAT IS OWED above it; on the phone the pane is the full-screen layer and the transcript is one swipe or one tap away. The row's content and its order are unchanged.**
                      bottom; "jump to now" returns to the end. One entry per row, one line each: — **RULED 2026-09-21 (the researcher, R68 „אופציה ב”): OLDEST FIRST stands (a thread reads opening → rationale → assessment → close), and the pane OPENS SCROLLED TO THE END, at now, as a chat does; the control is therefore "jump to the start" (`research.thesis.jumpToStart`), the thread headings are the anchors, and on a return the turns after `since` are marked.**
...
beside them, so a citation, its debate and the stream are in view at once. The context line stays a line. **RULED 2026-09-21 (the researcher, R68 „אופציה ב”): the THESIS is the centre at every width, as on the public page; the pane's tabs are TRANSCRIPT (default) · CITATIONS · GAPS · ANALYSIS · FRAMING · APPEALS, at the TOP of the pane — never a menu inside the page body; on the phone the pane is the layer a swipe reveals.**
...
the timestamp, the anchor mark (ATTRIBUTED, or not yet) **and, from 2026-09-19, the CITED mark when the capture's own `evidence` is set — this enumeration omitted it while :686 named it in the diff paragraph's tail, so a capture row drew nothing to explain the ringed dot region 3 puts six centimetres above it. Measured on the real corpus: 3 of 22 captures carry `evidence`, 0 of 21 diffs do**. Its COPY gives the citation token. A DIFF is the event: a card —
...
1  THE CONTEXT LINE   sticky: the scope (opened pages · every page) · the active filters as chips · the count
                      ...
                      **AND THE NAMES ARE THE READER'S, NOT THE DESIGN'S.** „עדשות” was this document's
                      ...
                      preference.
2  THE FILTERS        one row of chips, horizontally scrolling: PAGE (a picker from the read's own `pages` **RULED 2026-09-21 (the researcher, R68 board ט·ב): the row holds ROW FILTERS only — KIND · CITED · SINCE / UNTIL — under a visible label (`corpus.filtersLabel`); PAGE is the SUBJECT and is chosen in region 0, never a chip per page; a single-page view shows the active page as the header (region 3) and one link back to region 0 (`corpus.allPages`); the lens control is not drawn inside a single-page view. Reason: a page chooses WHAT is shown, the other chips narrow its rows, the lens changes the view — three controls in one row read as none.**
...
3  THE PAGE CARD      AMENDED 2026-09-18: it REPLACES the date axis and sits BELOW the filters. The page's **RULED 2026-09-21 (the researcher, R68 board ט·ב): the page card is the FIRST element of a single-page view, above the filters — a header that says the view is ONE page's.** **AND IT IS A HEADING ELEMENT, RULED 2026-09-21 (the researcher, R70 „I2 A”): the page's url is the single-page view's `<h2>` — `PageUrl weight='subject'` rendered a styled `<bdi>` and no heading at all, so the ruling above was honoured in size and not in structure — AND THE GENERIC PAGE TITLE („הארכיון של החוקרים”) IS NOT DRAWN ON A SINGLE-PAGE VIEW: the subject is the heading, and a generic `<h1>` above it is the very thing board ט·ב was drawn to remove (the researcher's note on it: „כותרת צריכה להגיד בבירור שזה דף אחד”). Region 0, which is about every page, keeps its own title.**
   WITH THE TIME      url, its interval, its record count, and the STRIP — captures as dots with cited ones
   STRIP              ringed, diffs as bars by chunk count, positioned by time with month labels. It IS the **RULED 2026-09-21 (the researcher, R68): past 15 months the strip labels YEARS only — a 42-month page smeared every month label into one line.**
                      scrubber the axis was, and it is ONE page's shape, which is why it is a heading here **RULED 2026-09-21 (the researcher): AND ITS SOURCE IS THE `pages` FACET'S OWN `shape`, NEVER THE ENTRIES THE VIEW RENDERS. The card's text line was already the facet's while its strip was the FILTERED, cursor-windowed array, so one element contradicted itself — measured on the real corpus: `?page=<corona>` drew „43 רשומות" over 16 dots and 16 bars, `&kind=CAPTURE` the same line over 16 dots and NO bars, `&kind=DIFF&cited=1` the same line over an empty strip. Ruling (g) below already forbids narrowing the strip by the significance gate; a FILTER is that mistake one level up and a WINDOW is it a second time. The facet is the right home and costs nothing: `listCorpus.ts` :97 loads it BEFORE the filter at :98 and the slice at :105, and `corpusReads.ts` :1295–:1312 already holds each page's captures and diffs when it builds the row — so §8 stands as written, the chips stay the one read's parameters, and the strip stops depending on `CORPUS_READ_LIMIT`. The facet carries DAYS and never geometry: no pixels (the 5px merge is the component's), no midpoint (a drawing rule), no sum where (e) says max; and `shape` is null on every row of a read that names no page, because region 0 draws no strip. THE SAME RULE REACHES region 4's „N מוסתרים", computed from that same window and honest only while a page fits one.**
                      ...
                      :56–:58); the strip answers that without a second element. **RULED 2026-09-19 (the researcher), on three options RENDERED from the real body at 375px and not argued in prose — the R58 method.** (a) THE CARD IS DRAWN ONLY WHEN `page` IS SET: a stream reached by `?cited=1`, `?since=` or `?until=` alone carries NO region 3, because there is no "the page", and an aggregate strip across pages contradicts this region's own "ONE page's shape"; the stream then begins under the filter chips. (b) A DIFF IS A NARROW BAR AT ITS INTERVAL'S MIDPOINT AND ITS HEIGHT ALONE IS THE CHUNK COUNT — "bars by chunk count" maps ONE magnitude to ONE axis, and a width spanning `before`→`after` adds a second axis, DURATION, which a diff does not have; measured on the real corpus, the spanning form let 28.6→5.8's 117 chunks swallow their neighbours. First ruled the other way and CHANGED the same day after both were drawn, which is the record of why. What it costs, stated because it was chosen and not stumbled into: a midpoint bar loses the visual link to its two endpoint dots, and any control restoring that is BEHAVIOUR, which a board never grounds. (c) "CITED" ON A CAPTURE DOT IS THE CAPTURE'S OWN `evidence`, never a diff touching it — measured: 3 of 22 captures carry evidence and 0 of 21 diffs, so the other reading rings nothing at all. (d) THE STRIP DRAWS TRUE ELAPSED TIME, GAPS INCLUDED, never compressed to where records exist: 78% of this page's 376 days are voids of 20+ days and the largest is 84 days, 22% of the strip's width — and a gap in the corpus is a gap in the EVIDENCE, so an instrument that compressed it would lie by omission. The Internet Archive's own year band, read on this very URL, draws 2003–2020 empty across three quarters of its width for the same reason. **THE FOUR DRAWING QUESTIONS THE SKETCH FOUND, RULED 2026-09-19 — and the researcher's own reading of the rendered options is part of the ruling: *„אני לא מזהה הבדלים משמעותיים"*, which is TRUE and MEASURED — at 375px the options differ by 1 to 3 pixels, so the choice is made on what survives a GROWING corpus and not on what is visible today, and a later seat is not to re-open it for looking the same.** (e) CAPTURES CLOSER THAN 5px MERGE INTO ONE DOT CARRYING THEIR COUNT. One day is 0.85px on a 319px strip and five pairs of this page's 22 captures sit 0.8px apart, so a 2.2px dot draws May 2022 as a smear. Merging draws the CIRCUMSTANCE — "four captures here" — where a smear draws nothing, and it does not compress time, which (d) forbids: only the MARK merges, never the axis. A merged dot is ringed when ANY capture under it is cited. **The same rule merges BARS**, whose midpoints collide identically — three of May's sit within 4.2px. **A MERGED BAR TAKES ITS GROUP'S LARGEST CHUNK COUNT, NEVER THEIR SUM, and is FULL-TONED when ANY diff under it passed the gate — both settled 2026-09-19 after the builder asked, because this clause was silent on each.** A sum would draw a magnitude no single change has: May's three are 50, 52 and 52, and summing them gives 154 against the page's largest real change of 125, so the tallest mark on the strip would correspond to nothing that happened. **Merging is the MARK and never the data** — the height goes on saying "a change of that size happened here" and the count says there are more. The tone mirrors the ring's ANY for the same reason: dimming a mark that contains a flagged change would hide the flagged one behind its quiet neighbours, which is the suppression (g) refuses. (f) THE BAR'S HEIGHT IS THE SQUARE ROOT of the chunk count, not a linear share and NEVER a minimum height. Linear maps 3 chunks to 3.5px against 125's 24px and loses the small change; a 3px FLOOR draws 1, 3 and 6 chunks at one height, which LIES about the very number (b) chose to show. A root is monotonic in the count, so "the height is the chunk count" still holds, and 3 chunks read at 6.3px. (g) THE STRIP DRAWS ALL THE PAGE'S DIFFS, the ones the significance gate hides included, in the LIGHT tone the gate's own rows use. **THE RESEARCHER'S OWN, asked for separately and answered „שאלה 4 - אני מעדיף א" — REVIEW declined to recommend on it, because it is a ruling about the GATE and not about drawing. THE PRINCIPLE BEHIND IT IS THE RESEARCHER'S, IN THEIR WORDS, and it is written here because it governs more than this mark: *„אני אוהב את האפור הבהיר שמראה מידע פחות חשוב באופן פחות מודגש ולא מעלים אותו מעיני המשתמש"* — DE-EMPHASISE, NEVER HIDE. Where a thing is judged less material, the strip draws it lighter and still draws it.** It follows this section's existing principle rather than extending it: the gate is "a filter that announces itself, not a suppression", and a strip that drew only the 8 of 21 that passed would BE the silent half — it would make the page's shape "the shape the classifier approved" and let silence decide, which :684 forbids in terms. Measured, it is not academic: drawing only the passed 8 removes EVERY low bar from this page's strip, so the strip would lose exactly the material that justifies the "13 hidden — show them" line beneath it.
4  THE STREAM         WHAT A FILTER RETURNS, and never a destination of its own (amended 2026-09-18 — see
                      ...
                      TAB (§26, and the UI plan's §10 :1217–:1223 — recomputed 2026-09-19, it read :1138 — a tab since the shell gained a pane, not a
                      sheet); **RETIRED 2026-09-21 (the researcher): "a page label tap adds the PAGE filter" — it was never built (the label is a bare `<bdi>` with no link, on both components) and board ט·ב made it contradictory: a page is CHOSEN in region 0 and NAMED by the header, never set from a row inside the view that is already filtered to it. IN ITS PLACE, THE RULE THE LABEL NOW ANSWERS TO — A ROW NAMES ITS PAGE ONLY WHEN THE VIEW SPANS PAGES. On a single-page view the card above already names it, so forty-three rows repeating one url is the redundancy this removes; §25's claims view NEVER spans pages and therefore never draws it.** Both row weights stand: inside ONE page
...
first (prosecutor §4 :87; researcher day :61–:62). A row: the claim's first words (in `.record-captured` — §26 :820's own rule applied, 2026-09-20: the words are the archive's extracted bytes, a document the platform did not author, and the thesis page's `#tr_` pane already draws them so) · **the page's label MOVED TO A HEADER, 2026-09-21 (the researcher): this view never spans pages (`page` is REQUIRED, :783), so a label on every row named one page twenty-six times while the view still could not say what it was about. It is drawn ONCE above the rows, from the body's own `page.url`, under §24 region 4's rule. THE HOLE, FILED RATHER THAN HIDDEN: `list_trajectories` carries the url on ENTRIES ONLY, so a window narrowed to nothing has no url anywhere in the body and the header vanishes exactly where a reader most needs it — the envelope is owed a top-level `page`.** · the
pattern as a strip of ticks — present, absent, present — across its captures (**RULED 2026-09-20, the researcher, on two options rendered from the real body: ONE TICK PER CAPTURE, each carrying its own instant in `data-`, drawn from the read's `captures[]` (§6.1 :248) and never one tick per flip span — a span names its first capture only, so a per-span strip drew 4 ticks for 22 captures; AND where the row is a GROUP — `claimCount` > 1, 13 of this page's 26 rows, 2 to 45 claims that moved as one unit — the first claim's words carry ONE COUNT WORD beside them, because the co-movement is the finding and a row showing one sentence of 45 hides it; the word is copy, drafted and frozen before it lands**) · **how many times it flipped
...
claim present or absent at each, each capture a link to its record, and the diffs in which it left or returned — both from the read's `captures[]` (§6.1 :248, RULED 2026-09-20). **AND THE GROUP'S MEMBERS, RULED 2026-09-20 (the researcher): where the row is a group, the sheet lists every claim that moved with it, each in `.record-captured` and each with ONE COPY control carrying `#tr_<trajectoryId>` — §4 :171–:173 gives a trajectory its chat-ready token and T2 :373 cites ONE `ClaimTrajectory.id`, never a pattern, so a group of 45 is 45 tokens and no token of its own; a single-claim row has one control. The label is `corpus.copyToken`, CALLED.**
...
## 26. THE RECORD SHEET, AND THE RECORD PAGES — **RULED 2026-09-19 (the researcher), two clauses this section had left to inference.** (1) **THE ARCHIVE'S BYTES TAKE `--font-serif`**, on all THREE surfaces this section names: the capture page's text version, the sheet's capture text, and the diff page's CURRENT chunks. It is the rule `docs/gf-ui-design-session-2026-09-16.md` §1.8 states — the serif's job is A DOCUMENT BROUGHT VERBATIM THAT THE PLATFORM DID NOT AUTHOR — and `.record-captured` (`globals.css`) already carries it on the thesis page's record pane; the record surfaces CALL that class and never re-spell it. Written here because until now the rule lived only in a dated design-session record and in built code, which the two-session protocol files under DESCRIBE and never DECIDE, so a builder reading this design found nothing. (2) **A RECORD PAGE IS NOT A THESIS PAGE**, verbatim: *„דף רשומה הוא לא דף תזה"* — so none of the three record pages carries the legal disclaimer, `COMPLIANCE.md` :92 naming a thesis page and a call page and nothing else. **`/corpus` LOSES ITS DISCLAIMER too** — *„יש להסיר את ההסתייגות הלא עקבית מ /corpus"* — which makes it consistent with `/theses`, from which the same reasoning removed it on 2026-09-19 (*"a list is neither"*). The public pages that carry it are exactly `/theses/[id]`, `/theses/[id]/versions/[v]` and `/call/[id]`.
...
and ONE link onward to the record's own page. **RULED 2026-09-19 (the researcher), and it settles a question this clause was read as leaving open: THE SHEET SHOWS THE CAPTURE'S TEXT IN FULL, exactly as the thesis page's right pane already does — *„יש לנו כבר היום תצוגה של הצילום בחלק הימני של תצוגת התזה"*. A corpus sheet carrying LESS than the surface that already ships would be a second, poorer answer to a question the platform has answered. REVIEW had offered a bare sheet and a clamped one; both were solving a problem this design had already solved, and are recorded here only so the reasoning is not repeated.** **AND ONE COMPONENT DRAWS A RECORD'S CONTENT, by the researcher's clean-code rule stated in the same breath — *„no duplicate code to produce same element"*.** `EvidenceRecordPane` takes an `EvidenceCitation` and a corpus row is not one, so the shared element is EXTRACTED and CALLED by both surfaces, never re-spelled beside them; the extraction edits `components/thesis/*`, which the UI plan lists as KEEP, so it lands DECLARED with this ruling as its authority. The precedent is ruling 2 of this same day — *"ONE MECHANISM, by clean code: every surface that names a provision calls one component and reads one catalogue"*. **WHAT BLOCKS IT IS A READ AND NOT A DESIGN.** Measured on the running body: `GET /api/thesis/:id` carries `content.text`, which is why the thesis pane can draw it; of the corpus reads, `/api/pages/:id/findings` carries a capture's `textHash` and no text, `/api/records/:fileHash` carries only the CITING VERSION's text, and `/api/pages/:id/diffs/:b/:a` is the ONLY public route holding a capture's bytes, as `before.text` / `after.text`. So this clause and A1 :1123 — which assigns the capture PAGE the findings read — both asked for a text no corpus route served. **CLOSED 2026-09-19, and the shape REPLACED the same day by a cold design review the researcher commissioned and accepted whole. THREE CLAUSES:** **(1) ONE COMPONENT IS `RecordContent`, NOT THIS PANE.** The two surfaces render ONE domain object — the record's BYTES — inside TWO context objects: a thesis CITATION (pin, verified, flag, argued, overObjection) and a corpus ROW (anchor, opinion, narrowed, citedBy, trackedUrlId). `RecordContent` takes the domain line, the dated heading and `content` AS A VALUE — `{kind:'CAPTURE',text}` or `{kind:'DIFF',chunks:[{side:'REMOVED'|'ADDED',text}]}`, plus LOADING and AWAITING — and each surface keeps its own marks around it. Extracting the PANE instead would force the corpus to synthesise a citation it does not have (`pin: null`, `verified: { notEvaluable }`) and the pane would then draw "not evaluable" as a reason for a row nothing evaluated — conditional accretion on day one. **And the shared element is smaller than it looks: of the pane's seven `.record-*` classes only `.record-captured` is defined at all.** **(2) THE SHEET READS FOR ITSELF WHEN IT OPENS; THE THESIS PAGE DOES NOT — AND THE REASON IS THE PIN,** not row counts. A citation's text is the text AT ITS PIN; a capture read by `(page, capture)` is the CURRENT extraction, so the thesis page cannot move to reading on open without changing what it shows until the read takes `textHash`. The thesis page ALREADY reads on a reader's act (its history's diff), so the two surfaces do not differ in kind. **(3) THE READ IS A RESOURCE** — `get_capture({url, capture, textHash?})` at `GET /api/pages/:trackedUrlId/captures/:capture` (evidence A4), never an optional argument on `list_findings`. **The ELEMENT is one; the CONTEXT and the FEEDING are each surface's own, and that is not duplication.**
...
3  FRAMINGS            list_framings (A4 :1432): every framing, oldest first — question · provision · author ·
                       ...
                       none yet, shown as such (§12 :1163) — **RULED 2026-09-21 (the researcher, R68 board ב): a framing with NO ROUND is shown muted as OPENED WITHOUT A ROUND (`research.framings.openedNoRound`); "has produced no thesis yet" is for a framing WITH rounds and no thesis. The platform records no link between framings — a refinement is not derived from adjacency (§21 :626).**
4  THE CORPUS          in numbers, from list_pages: pages surveyed · rows by outcome · stops pending (a fact) —
                       and the link to `/research/corpus` — **RULED 2026-09-21 (the researcher, R68 board ב): region 4 is ONE DOOR CARD — `research.corpus.open` as its title and ONE summary line (pages surveyed · captures acquired · rows not fetched · stops pending); the per-outcome breakdown lives beside each page on `/research/corpus` (§27 :873), not here.**
```
...
PUBLIC (no identity; PUBLIC_PAGE where a page is involved)
  ...
  /corpus/claims?page=                     the page + filters     GET /api/corpus/claims?page= (list_trajectories public; `page` REQUIRED — re-pointed 2026-09-20 (R65) to match :91 and §25 :783, which this table had missed on 2026-09-18)
  /pages/[trackedUrlId]/captures/[capture] page + timestamp       GET /api/pages/:id/captures/:capture · …/chain   (re-pointed 2026-09-19: one capture's own read, not the page's whole findings)
