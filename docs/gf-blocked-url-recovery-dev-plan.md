...
**Status:** Phase 1 ✅ DONE 2026-08-20 (uncommitted, branch `refactor/gf-evidence-shared-helpers`) — see
§5. Phases 2-5 not started. Supersedes the first draft (same date): the permission model and
...
```
on `Evidence` ([schema.prisma:193-234](../apps/glass-fortress/backend/prisma/schema.prisma#L193-L234)).
Additive, no backfill needed (every existing row correctly gets `[]`). Apply to staging first, verify,
then production following the same process already used for the `onChainTxHash` migration (`docs/
gf-evidence-integrity-dev-plan.md` §3.7 — direct `prisma migrate deploy`, verified post-apply before any
code depending on the column deploys).
