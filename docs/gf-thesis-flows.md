...
```
HISTORY(thesis)   every row that names the thesis, in time order, each attributed — **RULED 2026-09-20 (the researcher, R66: „Q1 transcript approved”): HISTORY(t) is a TRANSCRIPT of TURNS. A turn is one thing one voice said in one step: the RESEARCHER (a handle, `mine`), the MODEL (its name and prompt version, and who spent the call, A2 :1317) or the PLATFORM (a verdict, a gate). A stored row is one turn or several — a FramingRound is one; a PublicationAttempt is three (the rationale, the assessment, the verdict); a debate is its opening, its events and its close. Each turn carries its thread (the step and that step's id), a LINE — the turn's identifying DATUM in the record's own words (the question · the proposed sentence · the claim · the record's name · the gap's description · the strength grade · the note's first line), or null where the kind alone identifies the turn — NEVER a sentence the backend authors (RULED 2026-09-20 (the researcher, R66 „Q1 datum”): the page composes every phrase of ui §11's table from `body` through frozen strings, the chat reads the JSON, and the two name the same DATUM), and its body. Served whole, oldest first, by get_thesis_context (A4 :1476), `since` for the delta; get_framing and get_debate read their thread through the SAME builders, one spelling. The shape is A4 :1476's envelope; the seventeen turn kinds are listed there and nowhere else.**:
...
list_theses({})                                                       PUBLIC · ⚠️ to build — amended by docs/gf-ui-flows.md §7.1 (2026-09-15): optional `scope: 'mine' | 'all'`, default `mine`; `all` adds every researcher's theses, with handle and `mine`
  returns   anonymous: [{ thesisId, claim, provision, publishedAt, author: handle, contentHash }]
            ...
            researcher: their own theses — each with head, published, headIsPublished — **THE ENVELOPE, WRITTEN 2026-09-20 (R66) because this clause named FIELDS and not the SHAPE — the A4 :1112 / ui §6.1 :248 precedent; from the interface named and ONE real body per route, read 2026-09-20 on staging through the researcher’s own signed-in session in the pane (the local sign-in cannot complete: the auth redirect names staging): read from `listTheses.ts` `OwnEntry` :46, `AttributedEntry` :59, `ResearcherList` :61 and `publishedThesis.ts` `PublishedEntry`:** `{ theses: [{ thesisId, claim: string | null, provision: string | null, headVersionId: string | null, publishedVersionId: string | null, headIsPublished: bool, framingIds: string[], unarguedMentions: number, openGaps: number, author: handle, mine: bool }], published: [{ thesisId, claim, provision: string | null, publishedAt: ISO | null, author: handle, contentHash }] }` at `all` (`GET /api/research/theses`); at `mine` each `theses[]` entry lacks `author` and `mine`; anonymous, the `published` array ALONE and nothing else; `claim` is HEAD's; the four state WORDS of ui §11 :398–:399 (DRAFT ONLY · PUBLISHED = HEAD · PUBLISHED ≠ HEAD (n) · WITHDRAWN on <date>) were carried by NO field as served on 2026-09-20 (`headIsPublished` covers two arms, `versionsAhead` is computed by `publicationState` and NOT sent, a withdrawal is named by nothing) — **RULED 2026-09-20 (the researcher, R66 „Q1 transcript approved”; plan :779 names `listTheses.ts (the state through publicationState)`): every `theses[]` entry gains `state`, the SAME union `get_thesis_context` answers (A4 :1476: `{ kind: 'DRAFT_ONLY' } | { kind: 'PUBLISHED_IS_HEAD' } | { kind: 'PUBLISHED_BEHIND', versionsAhead } | { kind: 'WITHDRAWN', at, reason }`), through `publicationState`, one spelling; the page CALLS it and derives no state from `headIsPublished` and a count.** `all` without an identity refuses NO_RESEARCHER, a wiring DEFECT at the gated route — the
              framing attached, counts of unargued mentions and open gaps — and every published
              thesis as above
  closes    finding 30: every thesis tool needs an id nobody could list — and list_framings({}) GATED (2026-09-14, the step-20 record §2): every framing, oldest first, with its question, provision, author, the thesis it is attached to, its latest round and its CHOSEN claim verbatim; refuses nothing — **THE ENVELOPE, WRITTEN 2026-09-20 (R66) because this clause named FIELDS and not the SHAPE — the A4 :1112 / ui §6.1 :248 precedent; from the interface named and ONE real body per route, read 2026-09-20 on staging through the researcher’s own signed-in session in the pane (the local sign-in cannot complete: the auth redirect names staging): read from `listFramings.ts` `FramingEntry` :32:** `[{ framingId, question, provision: string | null, author: handle, thesisId: string | null, openedAt: ISO, rounds: number, latest: { sequence: number, type: 'PROPOSED' | 'ASSESSED' | 'CHOSEN' } | null, claim: string | null }]` — a bare ARRAY (no envelope key), ordered `createdAt` then id; `claim` is the latest CHOSEN round's, null until a choice; `[]` is an answer**
...
get_framing({ framingId })                                            GATED read · ⚠️ renamed
  returns   the framing and every round, verdicts included, and the thesis it attaches to — **THE ENVELOPE, WRITTEN 2026-09-20 (R66) because this clause named FIELDS and not the SHAPE — the A4 :1112 / ui §6.1 :248 precedent; from the interface named and ONE real body per route, read 2026-09-20 on staging through the researcher’s own signed-in session in the pane (the local sign-in cannot complete: the auth redirect names staging): read from `getFraming.ts` `GetFramingAnswer` :47, `ProjectedRound` :38, AS SERVED 2026-09-20:** `{ framingId, question, provision: string | null, thesisId: string | null, researcherId, rounds: [{ sequence: number, type: 'PROPOSED' | 'ASSESSED' | 'CHOSEN', content: object | null, malformed: bool, researcherId, createdAt: ISO }] }` — `content` A2 :1304–:1309's Json per `type`, `null` with `malformed: true` when the stored content is not an object; the author an ID, no handle. **RULED 2026-09-20 (the researcher, R66 „Q1 transcript approved”; plan :779 names `getFraming.ts (its rounds through the builder)`; A4 :1476's `P` and `T`): the read answers `{ framingId, question, provision: string | null, thesisId: string | null, by: P, turns: T[] }` — the FRAMING thread's turns (FRAMING_OPENED · ROUND_PROPOSED · ROUND_ASSESSED · ROUND_CHOSEN) from the SAME builder the transcript uses, in sequence, a malformed stored content reported as `malformed: true` on its turn and never as `{}`; `rounds` and `researcherId` are RETIRED with the builder, as `get_debate`'s `events` were (evidence A4 :1144)**; refuses NO_FRAMING as `{ error, code }` at 404 inside the prefix**
...
get_thesis_context({ thesisId, since? })                              GATED read · re-shaped — **THE ENVELOPE, RULED 2026-09-20 (the researcher, R66: „Q1 transcript approved”) on R66's first report, which found `history[]` served ids alone): FROM THE RULING AND THE INTERFACES (`getThesisContext.ts` `ThesisContext` :79, `VersionView` :56, `ResolvedMention` :45, `AnalysisState` :65; `thesisPredicates.ts` `GapEntry` :142; `thesisView.ts` `publicationState` :34; the row types of A2), a real body OWED once the transcript is built:** `{ thesis: { thesisId, provision: string | null, by: P, headVersionId: string | null, publishedVersionId: string | null, publishedAt: ISO | null, publicInterestStatement: string | null, createdAt: ISO, state: { kind: 'DRAFT_ONLY' } | { kind: 'PUBLISHED_IS_HEAD' } | { kind: 'PUBLISHED_BEHIND', versionsAhead: number } | { kind: 'WITHDRAWN', at: ISO, reason } }, head: V | null, published: V | null, unargued: string[], gapList: [{ gapId, readsAs: 'OPEN' | 'CITED' | 'REQUESTED' | 'CALLED' | 'CONCEDED' | 'DISMISSED', inForce: the ThesisGapDecision row (A2 :1321–:1330) WITHOUT `researcherId` — its `by: P` stands beside it; no researcher id anywhere in this body (RULED 2026-09-20, R66 „inForce approved”), by: P }], analysis: { state: 'NONE', fingerprint?: string } | { state: 'AWAITING_DERIVATION', name } | { state: 'CURRENT', fingerprint, analysisId, runAt: ISO, by: M, opinion: Json (A2 :1315–:1316) } | { state: 'STALE', fingerprint, latest: { analysisId, inputFingerprint, runAt: ISO } }, framings: [{ framingId, question, provision: string | null, by: P, createdAt: ISO }], history: T[], pages: [{ trackedUrlId, url }], owed: number, reviews: [E & { record: R | null, owedSince: ISO | null }] }` — **the UNION of HEAD's and PUBLISHED's cited pages, DEDUPLICATED BY URL (ruled 2026-09-21, same day): the centre draws HEAD's text with PUBLISHED one toggle away (plan :739 (iii)) and BOTH texts' chips resolve `pageId` from this one list, so a list covering only one version loses every link on the toggle. It is inherent rather than added work — `publishedThesis.ts` :314 already accumulates them as a `Map<url → trackedUrlId>`, so the union is that map filled twice — and a page named here that the version on screen does not cite is harmless, because `ThesisText` :49 looks up BY URL** — **`pages` ADDED 2026-09-21 (the researcher, R70 „option (a)”): the CITED pages, as the public body carries them (`publishedThesis.ts` :200, :372), because the citation chip's link onward is drawn from them (`ThesisText` :49 resolves each chip's `pageId` by matching `citation.record.url` against this list; §18 :575). **CORRECTED 2026-09-21, same day: this clause first also named §17 :559's THE PAGES, which the working view's centre does NOT draw — board ד2 has no such region (plan :739). The field is owed for the CHIP alone, and that is enough: without it every chip in the centre loses its way to the record.** and the gated door must not take a second read for a fact its own read can carry — ui §6.1 :241's precedent, where `list_corpus`' `page` gained `public: bool` for the same reason** where `P = { handle, mine: bool }` (a researcher — NEVER an id on the wire), `M = { model: string | null, promptVersion: string | null, spentBy: P }` (a model; null where the writer recorded none — the debate assessment records BOTH since document step 33 (`respondInDebate.ts` `assessAndRecord` :74–:75, the A2 :1317 debt PAID; seen on staging 2026-09-25 as `gemini:gemini-flash-latest` · `v3-document-record-and-assertions` — CONFORMED 2026-09-25, R83); a row written before it still reads null), `V = { versionId, text, claim, contentHash, createdAt: ISO, by: P, mentions: [{ kind: 'EVIDENCE', name, pin: string | null, argued: bool } | { kind: 'TRAJECTORY', name, pin: null, argued: false, resolves: bool, currency?: TrajectoryCurrency }] }` — **RE-SHAPED 2026-09-21 (the researcher, R70 „option (a)”), and the appendix is catching up with two clauses it already disagreed with rather than gaining a requirement: `mentions` is RESOLVED as T5 :811–:818 lists a resolved citation, the shape the PUBLIC body already carries and `publishedThesis.ts` already builds for the PUBLIC body at :302–:353 (`evidenceCitation` :406) — **CORRECTED 2026-09-21, same day, by a cold review: this clause first cited :249, which is `citationRefsByVersion` and builds the UNRESOLVED `CitationRef = { kind, name, pin }` (`types/thesis.ts` :209–:213) despite a docstring that says „citations”. The resolver is INLINE and SINGLE-VERSION (`where: { versionId: pin }` :303), so HEAD and PUBLISHED are two calls or one factoring, and no multi-version resolver exists to reuse. Naming the wrong function was this appendix's own warning turned on itself — what an implementation SERVES read off its docstring rather than its rows** — `[{ kind: 'EVIDENCE', name, pin: string | null, argued: bool, record: { url, capture? , before?, after? }, content: { kind: 'CAPTURE', text } | { kind: 'DIFF', chunks: [{ side, text }] }, verified: { verified: bool, captures: [{ capture, attributed: bool | null, anchoredHashMatchesDocumentHash: bool }] } | { notEvaluable: string }, flag: { flagged: bool, reasons: string[] }, overObjection: bool } | { kind: 'TRAJECTORY', name, pin: null, argued: false, resolves: true, claimText, url, transitions: number, current: bool } | { kind: 'TRAJECTORY', name, pin: null, argued: false, resolves: false }]` — **RULED 2026-09-25 (the researcher, R81 QC): `V.mentions` gains `{ kind: 'DOCUMENT', name, pin, argued, title, custody: 'HELD' | 'SEALED', verified: boolean (VERIFIED(d), document A6 :1529), flag: { flagged, reasons }, overObjection }`, HEAD and PUBLISHED alike; step 34 adds document §7's public fields to this arm and reshapes nothing** — **`currency` DROPPED FROM THE WIRE 2026-09-21, same day, on a cold review: it was written onto the `resolves: false` arm, where it cannot exist (a currency is a property of a trajectory that RESOLVED), and no reader consumes a mention's currency at all — `OwedStrip.tsx` :242 reads `review.state` from a thesis REVIEW, not from a mention. It remains the backend's internal signal for `resolves` (`getThesisContext.ts` :301) and stops there. With it gone the gated TRAJECTORY arm is EXACTLY the public `TrajectoryCitation` (`types/thesis.ts` :250–:251), which is what ONE citation shape was meant to mean**, for HEAD and for PUBLISHED alike. **WHY THE OLD SHAPE WAS WRONG AND NOT MERELY NARROW:** :1477 and ui §10 :367–:368 both say *resolved mentions*, and ui §11 :410 (the chip: *the record by page and date, the pin by its date, ARGUED or UNARGUED*) and :413–:415 (the CITATIONS tab: *the record (page, timestamps) · the pin · ARGUED with the verdict and „over objection" as a fact · FLAGGED*) have required the record since they were written. This envelope was written FROM THE INTERFACES (R66) and so recorded the CODE's narrower sense of „resolved" — `ResolvedMention`'s pin and argued — which is the failure mode ui §6.1 :248 is the PRECEDENT for — an envelope written from the interfaces and corrected against a real body; the principle it illustrates, stated here: an envelope read off an implementation states what is SERVED, never what is OWED. **The `V` shape is now the ONE citation shape, public and gated,** so the thesis column (ui §17, UI-5's `components/thesis/*`) is CALLED by the working view with no second read and no second chip — which is what the „אופציה ב" ruling of 2026-09-21 (ui §11 :431, §14 :486; plan :739, *„TEXT is the centre and not a tab"*) requires, TEXT being HEAD's text (§11 :410–:412) that no public read carries. §10 :369–:371's CLOSED LIST OF READS is therefore unchanged and unbroken: this adds a FIELD to the one read, never a read**, and a TURN `T = { kind, id, at: ISO, thread: { step: 'FRAMING' | 'VERSION' | 'DEBATE' | 'ANALYSIS' | 'GAP' | 'PUBLICATION' | 'WITHDRAWAL' | 'NOTE', id }, by: { voice: 'RESEARCHER' } & P | { voice: 'MODEL' } & M | { voice: 'PLATFORM' }, line: string | null, body: B }` — `line` is the turn's identifying DATUM verbatim (RULED 2026-09-20 (the researcher, R66 „Q1 datum”); §9 :974): FRAMING_OPENED the question · ROUND_PROPOSED the proposed sentence · ROUND_CHOSEN and VERSION the claim · DEBATE_OPENED the record's name (`url` and its dates) · ANALYSIS the strength grade · GAP_DECISION the gap's description · NOTE the first line; null for ROUND_ASSESSED, RATIONALE, ASSESSMENT, RESPONSE, DEBATE_CLOSED, PUBLICATION_RATIONALE, PUBLICATION_ASSESSMENT, PUBLICATION_VERDICT and WITHDRAWAL, whose kind and `body` name them; never a composed sentence, in no language — with SEVENTEEN kinds and their bodies, a closed union — FRAMING_OPENED `{ question, provision, fromRunId: string | null }` (researcher) · ROUND_PROPOSED `{ framing, elements: [{ element, records: string[] | 'MISSING' }] }` (researcher, A2 :1305) · ROUND_ASSESSED the ASSESSED content with its marks (model, A2 :1306–:1309) · ROUND_CHOSEN `{ claim, provision, elements, restatedBy: versionId[] }` (researcher) · VERSION `{ text, claim, contentHash, parentVersionId: string | null, mentions: the version's STORED mention rows `[{ versionId, kind: 'EVIDENCE' | 'TRAJECTORY', name, contentVersionHash: string | null, debateSessionId: string | null }]` — NOT `V`'s resolved `{ pin, argued }` shape (`thesisTranscript.ts` :115–:120; RULED 2026-09-20, R67 Q-D, on run B's real body), citationsVsParent: { added: string[], repinned: string[], dropped: string[], carried: string[] } }` (researcher; the text diff to the parent is the page's ONE derivation, T6 :900) · DEBATE_OPENED `{ sessionId, record: { url, capture } | { url, before, after }, pin }` (researcher) — **RULED 2026-09-25 (the researcher, R81 QB): `record` gains a third form, `{ commitment, title }`, the title being document A2 :1271's researcher-asserted title; for a document the title is this turn's `line`** · RATIONALE `{ text }` (researcher) · ASSESSMENT `{ hasSubstance, substanceGaps, verdict, objection, assessment, malformed: bool }` (model, the event's JSON PARSED, `malformed` as get_framing reports it) — **RULED 2026-09-25 (the researcher, R81 QA): the body gains `assertions: [{ researcherClaim, quoteVerified, whatEvidenceShows, phraseVerified, phraseVerifiedReason: string | null }] | null` — **CONFORMED 2026-09-25 to document flows §3 :363 ("recorded UNCHECKED, with the reason"), which T1 :261–:262 points to: the reason rides beside an UNCHECKED verdict, as the framing audit's does, and is null beside PRESENT or ABSENT (R82)**: each assertion the debate's assessor names, a span of the citing passage and the phrase it attributes to the record, verified as the framing round verifies (A2 :1307–:1308, T1 :256–:261); `null` on a row written before this ruling, NEVER `[]`, which would read as an assessor that checked and found nothing** · RESPONSE `{ text }` (researcher) · DEBATE_CLOSED `{ outcome: 'PROMOTED' | 'ABANDONED', overObjection: bool, evidenceFileHash: string | null }` (platform) · ANALYSIS `{ analysisId, inputFingerprint, current: bool, opinion }` (model) · GAP_DECISION `{ gapId, description, sequence, decision, citedName, request, callItem, reason, earlier: [{ sequence, decision, at: ISO }] }` (researcher — a REQUESTED request is the researcher's approved text, T4 :670–:672) · PUBLICATION_RATIONALE `{ attemptId, rationale }` (researcher) · PUBLICATION_ASSESSMENT `{ attemptId, assessment, verdict }` (model) · PUBLICATION_VERDICT `{ attemptId, outcome: 'PUBLISHED' | 'REFUSED', refusedBy: string[] }` (platform) · WITHDRAWAL `{ versionId, reason }` (researcher; GATED, T6 :916) · NOTE `{ text, on: 'THESIS' | 'FRAMING' }` (researcher). Order: `at`, then the thread, then a row's own turn order (rationale before assessment before verdict), then id; `since` strictly after `at`. **A kind absent from this list is not in the transcript; a kind added (document flows' ARRIVED) is added HERE first.** Refuses NO_THESIS as `{ error, code }` at 404 inside the prefix — **`owed` AND `reviews` ADDED 2026-09-22 (the researcher, R71): THIS THESIS's entries of REVIEWS ride the read's OWN envelope, and the working view's second read of `/api/research/reviews` (`ResearchThesis.tsx` :99, introduced by chunk 7a) is DELETED.** **THE SHAPE MIRRORS :1523 DELIBERATELY** — `owed: number` is the COUNT and `reviews` the entries, `E` being :1523's own union (`FLAGGED` · `STALE_TRAJECTORY` · `UNARGUED`, `ARRIVED` later), so ONE NAME KEEPS ONE MEANING across the two doors. **THE ROW IS `E & { owedSince }` AND NOT :1523's FULL ROW — RULED 2026-09-22 (the researcher, R71 „approve B”), and the first spelling of this clause, written the same morning, was WRONG: it said `reviews: E[]` while claiming to MIRROR :1523, whose rows are `E & { owedSince, material: M, author, mine }`.** It silently dropped four fields and grounded itself on ZERO EXTRA QUERIES, which §11 :404 then contradicted by requiring *„each with its material”*. **`owedSince` JOINS because the rows already carry it** — still zero extra queries. **`material` does NOT**: FLAGGED's `M` needs `moved`, `cause`, `decision` and `current.chunks`, which are evidence-side rows this read never loads, so carrying it here would cost the queries this step spent itself buying back. **CORRECTED 2026-09-22, the same day, on the DEVELOPER's cold read of the ROWS (the researcher, R71 „approve c, rule the record in”): `owedSince` IS NOT FREE ON EVERY ARM, and the clause above asserted it of the FIELD without checking it PER ARM.** `thesisReviews.ts` :228 computes FLAGGED's as `later(published, earliest)`, where `earliest` folds **`decision.at`** (:211 `latestDecisionOf`) and **`material.movedAt`** (:207 `movedFrom` over :202 `recordRowOf`) — **the very material this clause excludes**, at 3–6 singular reads per flagged citation, an N+1 in the number of flags. **SO THE ROW IS PER KIND, and each pairing is fixed:** FLAGGED `{ record, owedSince: null }` · UNARGUED `{ record, owedSince }` · STALE_TRAJECTORY `{ record: null, owedSince }`. UNARGUED's date is `at.headCreatedAt` (:196) and STALE's is `later(citing, currency.latestComputedAt)` (:259) over a currency `ResolvedCitations.trajectories` already carries — both FREE. `record` is free on both arms that have one, from the `records` map `citationsFrom` already resolves. **`publishedAt` alone was REFUSED for FLAGGED**: `owedSince` is the LATER of publication and the reason's moment, so publication alone is a LOWER BOUND and the card would claim a flag had been open LONGER than it has — a false date on a forensic surface is worse than none. **A UNION'S FIELD IS ONE THING PER ARM, AND THE CHECK IS PER ARM.** **FLAGGED's material AND its date are the CITATION SHEET's** (§11 :404 as amended today), which is where §11 already sends a reader for a record's detail. `author` and `mine` are :1523's because that envelope spans theses; this one is ONE thesis, whose handle the context line already draws. **A shape claimed to MIRROR another is checked against that other's ROWS, in the same turn, or the claim is not made** — an envelope read off a NAME states what is served, never what is owed. An earlier draft wrote `owed: ReviewEntry[]`, which would have put a COUNT and an ARRAY behind one name on two envelopes parsed by one parser (`types/research.ts` :557 types it `number`) — caught before it landed, and recorded because the appendix's own `V` ruling is the precedent for ONE shape. **IT IS PER-THESIS, NEVER `REVIEWS(researcher)`** — A3 :1408–:1410 scopes that predicate to theses the caller AUTHORS, while §11 :407–:408 requires the same entries on a COLLEAGUE's thesis with the command labelled as the author's; a shared builder must not carry the author scoping with it. **ZERO EXTRA QUERIES** — FLAGGED from `flaggedFor` over PUBLISHED's mentions, STALE_TRAJECTORY from the currency the citation resolver already returns, UNARGUED from the body's own field, all already loaded. Ground: ui §6.1 :241 (`list_corpus`' `page` gained `public: bool` *„without a second read”*), T6 :876–:877 (*„repeated here so one read answers the question”*), and A3 :1413–:1414 (*„every predicate is computed on read and none is stored”*) — so this is a FIELD on the one read, and ui §10 :370–:371's CLOSED LIST OF READS is unchanged and unbroken. The alternative, §10 gaining a third read, cost a read over EVERY thesis on the platform to keep one row
...
list_thesis_reviews({})                                               GATED read — amended by docs/gf-ui-flows.md §7.1 (2026-09-15): optional `scope: 'mine' | 'all'`, default `mine` — **THE ENVELOPE, WRITTEN 2026-09-20 (R66) because this clause named FIELDS and not the SHAPE — the A4 :1112 / ui §6.1 :248 precedent; from the interface named and ONE real body per route, read 2026-09-20 on staging through the researcher’s own signed-in session in the pane (the local sign-in cannot complete: the auth redirect names staging): read from `thesisReviews.ts` `ThesisReviewList` :66, `FlaggedMaterial` :37, `StaleMaterial` :48, `UnarguedMaterial` :55 and `thesisPredicates.ts` `ReviewEntry` :448 (**CORRECTED 2026-09-22, R71**: this clause said :433 and the declaration is at :448, printed whole before the correction):** `{ owed: number, reviews: [E & { owedSince: ISO, material: M, author: handle, mine: bool }] }` at `all` (`GET /api/research/reviews`; at `mine` without `author` and `mine`), oldest first, where `E` is one of `{ kind: 'FLAGGED', thesisId, name, versionId, mentionId, reasons: ('WITHDRAWN' | 'NOT_CITATION_CURRENT' | 'AWAITING_DERIVATION')[], command }` · `{ kind: 'STALE_TRAJECTORY', thesisId, name (the trajectory id), citedOn: [{ versionId, published: bool }], state: 'PINNED_IS_LATEST' | 'RECOMPUTED_AGREES' | 'RECOMPUTED_DISAGREES' | …, command }` · `{ kind: 'UNARGUED', thesisId, name, versionId, mentionId, command }` and `M` by kind: FLAGGED `{ versionId, record: R, pin: { hash, chunks: ContentUnit[] }, current: { hash, chunks } | null, moved: { entered, left } | null, cause: Cause[], decision: LatestDecision | null }` · STALE_TRAJECTORY `{ citedOn, cited: { claimText, url, finalState, changes: Observation[], computation: { id, computedAt } }, currency: TrajectoryCurrency }` · UNARGUED `{ versionId, record: R, pin }`; `R` is evidence A1's name `{ url, capture } | { url, before, after }` — **CONFORMED 2026-09-26 (R86, to R81 QB at evidence :1123, "ONE shape wherever a record is answered"): `R` has the third form `{ commitment, title }` — a FLAGGED DOCUMENT mention's `record`, its `cause` the document EXTRACTOR form of evidence A4 :1146 (R85 Q-D); a reader parses `record` BEFORE `cause`**; ARRIVED is not built (document step 32). `{ owed: 0, reviews: [] }` is an answer; NO_RESEARCHER is a wiring DEFECT at the gated route**
