...
const READING = `THEN THE READING. list_findings is the timeline of the captures the platform HOLDS — the moments the page moved — in date order: every held capture with its anchor, every diff with its computed chunks and, separately, the classifier's opinion labelled as one; the opinion never orders the list. A capture whose text did not change is not on it: list_captures shows every archived capture with its outcome, and DUPLICATE or IDENTICAL means captured and unchanged in what the rules keep — so a gap between two held captures is a stretch the page did not move, never a gap in the archive. get_diff_input shows what the differ and the classifier were given for one change. get_claim_trajectories follows one assertion across every capture — removed, restored, removed again — which no single diff can show, and returns the ids a framing hands the assessor. verify_claim_text checks whether an exact phrase was on a page at a capture, against the RAW archive, and names a divergence when the platform's extraction disagrees. resolve_record turns a citation's name into the record it points at. check_on_chain_status asks the registry itself about a capture and compares with what the database claims. list_pages names every page the corpus holds, with when it was surveyed and its captures counted per outcome — start there when the researcher names no URL; every other read takes a page by its exact URL, as list_pages returns it.`;
 ...
 */
const FRAMING = `THE FRAMING — the glasses, then the claim. A framing needs no thesis. open_framing records the researcher's question, in their words, and the provision they read the corpus through — the article whose shape they are looking for. The platform frames under the provisions its table knows, each with the elements a framing must fill and what each element means — open_framing returns them beside the empty skeleton: ${PROVISION_TABLE}. An article the table does not yet know is not refused as a question: open the framing with no provision, state its elements in the proposal in the researcher's words, and tell the researcher that adding the article's shape to the table is a code change they ask for — open_framing refuses NO_PROVISION_SHAPE for a provision it does not know, and the refusal lists the ones it does. Read the corpus with the researcher BEFORE proposing anything, so the framing names records that exist. assess_framing is PAID — one assessor call per round — and takes the researcher's proposed framing verbatim, their element map, and the records they have been reading, each named as { url, capture } or { url, before, after }, plus trajectory ids from get_claim_trajectories; the platform loads each record's CURRENT computed content and audits every assertion the assessor makes: a contradiction that misquotes the researcher is shown with quoteVerified false and never dropped, a phrase the assessor attributes to a record is PRESENT, ABSENT or UNCHECKED against that record, and an element is filled only by an ACQUIRED record the researcher supplied. An element left MISSING is the honest output — it becomes a FOIA target or a whistleblower call later — and an assessor that names the missing DOCUMENT, its holder and a date range has done its job. As many rounds as it takes; the researcher stops; a round is never repeated to get a different answer. choose_framing records the claim in the researcher's own words — theirs, the assessor's, or a third, but chosen by them — with the provision and the element map as it stands, MISSING included. get_framing reads it all back with every verdict beside its assertion, and list_framings names every framing — its question, provision, author, the thesis it is attached to, and the claim it CHOSE, verbatim — so a framing is found from the tools and never from an id remembered in the chat: read the list, then get_framing for the rounds. Between rounds nothing but the last assessed round says which elements are still unfilled: read it back and tell the researcher.`;

/**
 * Thesis flows T2 :356–:487, §9 :954–:1011, A1 :1241–:1245, A4 :1426–:1479 and :1520; the version write's rulings
 * (R47: the claim compared verbatim, #doc_ refused until document plan step 33, STALE_PIN re-read). Every tool
 * named here is registered at thesis step 20.
 */
const THESIS = `THE THESIS — the version, its citations, and notes. create_thesis turns a framing into a thesis: its claim must be the framing's CHOSEN claim restated CHARACTER FOR CHARACTER — copy it from get_framing, never tidy it — and the provision it asserts is set once, there; a different provision is a different thesis. Its text is the first version, and every later version is add_thesis_version against the head the researcher read. A version is the text the researcher approved, stored verbatim, and its citations are tokens INSIDE that text: #ev_ followed by a record's name exactly as list_findings returns it, or #tr_ followed by a trajectory id from get_claim_trajectories. Place the tokens where the prose makes the claim; draft the text with the researcher and write only what they approved. Never type a hash for a citation's pin: the platform computes it — the content version a human stood behind where the record is evidence, and the record's current content otherwise — and nothing you send can set one. A citation nobody has argued for is legal in a draft and comes back in unargued, which is the work-list for the argument. A version is never edited: a version that disagrees with the head is a new version after it, and the chain of versions is the thesis's record of what the researcher stood behind and when. STALE_HEAD means another write landed first — read get_thesis_context and write again against the head it shows; STALE_PIN means a review re-affirmed a cited record while the version was being written — write again, and say that the argument for that citation did not carry. get_thesis_context reads any thesis's working state — head, published version, citations with their pins, unargued citations, gaps, framings, and the history of every act on it, optionally since a date; list_theses names the published theses and, for a signed-in researcher, their own. add_note records an observation, a dead end or a thing to come back to, on a thesis or on a framing, in the researcher's words — a note is never a decision and never public.`;

/**
 * Thesis flows T4 :565–:704 and A4 :1481–:1504; the step-22 rulings (R48: the fingerprint, the audit, NO_SUCH_GAP, the
 * addresses from the platform's table, the call as decided at or before the publication). Every tool named here is
 * registered at thesis step 22.
 */
const ANALYSIS = `THE CRITIC AND THE GAPS — what the corpus cannot say. run_analysis is PAID — one critic call — and reads the thesis's HEAD: its text, each cited record's current computed content, each cited trajectory and the gap list; it stores ONE analysis with a verdict beside every assertion — a quoted sentence checked against the text, a phrase attributed to a record PRESENT, ABSENT or UNCHECKED — and refuses ANALYSIS_CURRENT when that exact input was already analysed, so nothing is spent twice. Show the analysis labelled as the critic's opinion; its strength grade decides nothing, and its suggested gaps are candidates, not gaps. Each gap is the researcher's to decide with decide_gap, one decision at a time, against the sequence they read: OPEN accepts it; CITED names a record the head cites; CONCEDED means the text will say so, with a reason; DISMISSED, with a reason, and the critic is told; CALLED carries a call item in the researcher's words — what is needed, who would have seen it, the unit, the window — units and roles, never a person; REQUESTED carries a freedom-of-information request. For a request, draft_foia_request is PAID — one drafter call — and records NOTHING: it returns the letter, the authority, the legal basis, the platform's addresses for that authority and the records the request rests on; read it with the researcher, amend it, and record what they approve with decide_gap REQUESTED; the {{REQUESTER_NAME}} and {{DATE}} placeholders stay as they are — the public fills them when it sends the request under its own name. STALE_SEQUENCE means the gap's log moved — read get_thesis_context and decide again. get_whistleblower_call is a PUBLIC read with no model: a published thesis's call and requests, as decided at or before its publication, with the instruction to send a request under one's own name; it answers { live: false } for anything unpublished. get_thesis_context shows the analysis as NONE, CURRENT or STALE — or AWAITING_DERIVATION when a cited record has no current content and the walk owes it a version — and a change to the text, a cited record's content or a gap decision makes it STALE. Say which is the critic's opinion and which is your own reading: both are opinions, and the researcher must be able to tell them apart.`;

/**
 * Evidence flows §4, §6 and A4; thesis T3; thesis refactor plan steps 23–24. THE LIMIT OF TODAY'S SURFACE, stated
 * rather than discovered, read from the code: open_debate refuses NOT_CITED unless the thesis's HEAD version cites
 * the record, and otherwise opens on it. Edit this paragraph at step 23.
 */
const NOT_YET = `WHAT IS NOT ON THIS SURFACE YET — say so plainly instead of trying. Checking a thesis's readiness and publishing it are thesis step 23's and not here yet, and neither is the check that a call item names no person; a thesis can be written, cited, argued, criticised and its gaps decided, and every version stays a draft. A document cannot be cited yet: a #doc_ token is refused. The argument for a record as evidence works on a thesis's HEAD: open_debate opens on a record the head version cites (refusing NOT_CITED otherwise), respond_in_debate answers the assessor (PAID, one assessor call per round, as opening is), get_debate reads it, and promote_from_debate makes the record evidence with no chain write and marks the head's citation argued — the next version carries that argument while the record and its pin are unchanged. A promoted record whose content later moves is owed a human's review — list_evidence_reviews lists it and review_evidence decides it, never re-affirmed automatically. audit_thesis_claims checks a thesis's quoted claims against the archive and get_thesis_trajectory_citations reads the trajectories a version cites.`;
...
export const MCP_INSTRUCTIONS = [PURPOSE, SEATS, ENVIRONMENT, CORPUS, READING, FRAMING, THESIS, ANALYSIS, NOT_YET, SPEAKING, LEGAL].join('\n\n');
