...
/** Every provision the table knows, with its element shapes — read from the code, the one spelling. */
const PROVISION_TABLE = Object.entries(PROVISIONS)
  .map(([provision, elements]) => `${provision} → ${elements.join(', ')}`)
  .join('; ');
