 ...
 *
 * It is the same rule `corpusReads.opinionOf` holds one register over, for the
 * same reason and in the same words: absent is a FACT, whole is a derivation with
 * its provenance, and between them is nothing the design names.
 */
function chunksOf(stored: Prisma.JsonValue, pair: string): StoredChunk[] {
