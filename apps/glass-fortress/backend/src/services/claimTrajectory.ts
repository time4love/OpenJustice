...
//
// Presence is therefore tested against each capture's stored CURRENT `text` —
// the derivation every diff is cut from (docs/gf-interaction-flows.md A2) — NOT
// against a model's reading of it. The diffs' chunks are used only to DISCOVER
// which claims are worth following; the trajectory itself is a string search.
// ---------------------------------------------------------------------------
 ...
 *
 * v3 took candidates from CURRENT(diff)'s computed chunks (evidence step 11b).
 *
 * v4 (R45, 2026-09-13) moved PRESENCE from Readability's `fullText` to the
 * current `text` — the register those chunks have been cut from since v3, so
 * candidates were being searched in a different rendering from the one they
 * came out of. Which captures a claim is present at changed (corona: 28
 * trajectories gained, 7 reshaped, 0 lost), so no v3 computation describes the
 * corpus under v4.
 */
export const DETECTION_VERSION = 'v4-presence-in-current-text';
...
/**
 * Everything needed to identify the state, WITHOUT touching snapshot text.
 *
 ...
 * state has not moved. Loading the text here to decide whether we need the text
 * would defeat it.
 */
/**
...
/**
 * The texts of a content version's `chunks`, read defensively.
 *
 * `chunks` is `Json`, so the compiler knows nothing about it and a malformed row
 * must not take a detection pass down with it. Anything that is not an array of
 * `{ text }` contributes no candidates — the same answer an empty diff gives,
 * and one the pass already reports through `candidatesUnmatched` rather than
 * swallowing.
 */
function contentChunksOf(chunks: unknown): string[] {
...
async function loadDetectionInputs(url: string, minClaimLength = 0) {
  ...
  const snapshotMeta = (
    await prisma.urlSnapshot.findMany({
      ...
      select: { snapshotDate: true, waybackTimestamp: true },
    })
  ...
  // REBASED AT EVIDENCE STEP 11b ONTO THE CONTENT VERSION. Candidates were the
  // CLASSIFIER'S ITEMS — its Hebrew summaries, each with an `exactQuote` — and
  // those columns left the diff row. They come from CURRENT(diff)'s COMPUTED
  // chunks now: the differ's own segments, the page's own text.
  //
  // A BETTER SOURCE, NOT A SUBSTITUTE. The rule above says extraction is trusted
  // only to decide WHAT to look for, never whether it is there — and a model's
  // paraphrase was always the weaker way to decide it, which is why the pass
  // records `candidatesUnmatched` at all. A candidate drawn from a chunk is
  // verbatim by construction.
  //
  // The newest version per diff, which is CURRENT for a corpus the walk has
  // re-derived; a diff still AWAITING_DERIVATION contributes nothing, and that is
  // the honest answer for content nobody has computed.
  const diffs = await prisma.urlVersionDiff.findMany({
    where: { trackedUrlId: tracked.id },
    select: {
      contentVersions: { select: { chunks: true }, orderBy: { derivedAt: 'desc' }, take: 1 },
    },
  });

  const candidates = new Map<string, string>();
  for (const diff of diffs) {
    for (const chunk of contentChunksOf(diff.contentVersions.at(0)?.chunks)) {
      const normalised = normaliseClaim(chunk);
  ...
  return { trackedUrlId: tracked.id, url, snapshotMeta, candidates, sourceStateHash };
}
 ...
 * a caller not to cache an experiment: `detectAtClaimLength` detects over a
 * candidate set no scan produced, and it cannot reach the writer because the
 * writer demands the cacheable type. (A third type, for a measurement's
 * `sourceStateHash: null`, retired with its only user — see the RETIRED block
 * below.)
 */
...
async function detect(inputs: DetectionInputs) {
  ...
  const rows = (
    await prisma.urlSnapshot.findMany({
      ...
      ...TRAJECTORY_CAPTURE_SCAN,
      // THE REGISTER PRESENCE IS TESTED AGAINST: the capture's CURRENT `text`,
      // the derivation every diff — and so every candidate — is cut from. One
      // select, one column; the choice this line makes is what the pin hashes.
      select: { snapshotDate: true, waybackTimestamp: true, text: true },
    })
  ...
  const snapshots = rows.map((r) => ({
    ...
    // COMPUTED, not read: the viewer URL `recordCapture` writes for an archived
    // capture, reproduced from the same two inputs, so no detection function
    // depends on a legacy column.
    snapshotUrl: viewerCaptureUrl(r.waybackTimestamp, inputs.url),
    normalisedText: normaliseClaim(r.text),
  }));
...
// ---------------------------------------------------------------------------
// THE DETECTION LAYER SWITCH WAS RETIRED AT R45, WHOLE (2026-09-13).
//
// Presence was tested against a LAYER chosen by one constant: EXTRACTION
// (`fullText`, Readability's article) or DOCUMENT (`text`, the current
// derivation). Production read EXTRACTION; `compareDetectionLayers` and its
// script `forensics:compare-detection-layers` measured the move before it was
// made, detecting twice over one candidate set and writing nothing, and a
// third input type — `sourceStateHash: null` — kept that measurement from ever
// reaching the cache writer.
//
// THE MEASUREMENT WAS TAKEN, which is what makes this a retirement rather than
// a loss. Staging, deployment cd0e6def @ 69072a1, 2026-09-13:
//
//   corona  200 candidates · EXTRACTION 172 trajectories / 28 unmatched · DOCUMENT
//           200 / 0 · lost 0 · gained 28 · reshaped 7 (22 captures)
//   rtmag   0 candidates (1 capture) · walla 0 candidates (4 captures)
//
// The 28 gained were exactly the 28 unmatched: candidates cut from `text` that
// Readability's article never contained. The FDA sentence was among them — the
// step-19 walk's defect (a trajectory asserting it absent at 20220805053301
// where the page held it) — and the seven reshaped were headings and a banner
// whose flips under Readability followed its article boundary, not the page.
// Detection now reads `text` in `detect`'s own select; DETECTION_VERSION moved
// v3 → v4 with it. With the comparison gone its null-hash input type had no
// user and went too; the reasoning it carried stands above `DetectionInputs`.
// ---------------------------------------------------------------------------

// ---------------------------------------------------------------------------
// THE CANDIDATE-SOURCE COMPARISON WAS RETIRED AT EVIDENCE STEP 11b, WHOLE.
//
// It measured what each candidate source would make DETECTABLE before the
// expensive one was paid for: a CLASSIFIED arm over the classifier's items
// against a DOCUMENT_CHUNKS arm over raw chunks, with the asymmetry stated —
// it could cancel the spend, never justify it. The classified arm read
// `deletedText` / `addedText`, and those columns left the diff row: everything
// derived from the two texts is a version's (evidence A2).
//
// THE MEASUREMENT WAS TAKEN AND IS RECORDED, which is what makes this a
// retirement rather than a loss. `docs/gf-candidate-source-measurement-2026-08-30.md`
// says the free option survives and asks not to be re-measured. The detector
// below now takes its candidates from CURRENT(diff)'s computed chunks — the
// arm that won — and `DETECTION_VERSION` moved v2 to v3 to say that the
