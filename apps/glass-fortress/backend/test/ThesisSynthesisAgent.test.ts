...
describe('ThesisSynthesisAgent', () => {
  ...
  describe('synthesize()', () => {
    it('returns a valid ThesisSynthesisOutput when the LLM responds correctly', async () => {
      ...
      const result = await agent.synthesize('vaccine adverse event suppression', CORPUS_FIXTURE, { trajectories: [], coverage: [], omittedGroups: 0 });
    ...
    it('rejects a response whose narrativeBody cites a footnote with no matching citations entry', async () => {
      ...
      await expect(agent.synthesize('test', CORPUS_FIXTURE, { trajectories: [], coverage: [], omittedGroups: 0 })).rejects.toThrow(/\[3\]/);
    });
    ...
    it('rejects a response with a citations entry never referenced by a [^n] marker', async () => {
      ...
      await expect(agent.synthesize('test', CORPUS_FIXTURE, { trajectories: [], coverage: [], omittedGroups: 0 })).rejects.toThrow(/\[9\]/);
    });

    it('passes topic and corpus to the LLM', async () => {
      getMockInvoke(agent).mockResolvedValueOnce(VALID_RESPONSE);
      const topic = 'EUA suppression in vaccine campaigns';
      await agent.synthesize(topic, CORPUS_FIXTURE, { trajectories: [], coverage: [], omittedGroups: 0 });
    ...
    it('includes key figures in the corpus block sent to the LLM', async () => {
      getMockInvoke(agent).mockResolvedValueOnce(VALID_RESPONSE);
      await agent.synthesize('test', CORPUS_FIXTURE, { trajectories: [], coverage: [], omittedGroups: 0 });
    ...
    it('truncates long summaries to 500 chars in the corpus block', async () => {
      ...
      await agent.synthesize('test', corpusWithLong, { trajectories: [], coverage: [], omittedGroups: 0 });
    ...
    it('handles a corpus with no key figures gracefully', async () => {
      ...
      await agent.synthesize('test', noFigures, { trajectories: [], coverage: [], omittedGroups: 0 });
    ...
    it('throws if the LLM returns a schema-invalid response', async () => {
      ...
      await expect(agent.synthesize('test', CORPUS_FIXTURE, { trajectories: [], coverage: [], omittedGroups: 0 })).rejects.toThrow();
    });
