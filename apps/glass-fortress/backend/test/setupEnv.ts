...
// The chain variables are NOT hidden here. A deletion in `setupFiles` is undone
// by the test file's own `@prisma/client` import, which reloads `.env` — see
// test/chainIsolation.ts for the rule and test/setupNoRealChain.ts for where it
// is applied.
