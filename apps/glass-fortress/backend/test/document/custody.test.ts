...
interface Predicates {
  ...
  recomputable: (document: DocumentRow, shed: ShedRow | null, bytes: Uint8Array | null) => boolean;
}
...
describe('A3 :1360-:1363 — RECOMPUTABLE(d), by mode', () => {
  it('HELD recomputes from storage, forever: sha256(bytes) = docId', async () => {
    const { recomputable } = await predicates(['recomputable']);
    const bytes = new Uint8Array([1, 2, 3]);
    const { createHash } = await import('node:crypto');
    const name = '0x' + createHash('sha256').update(bytes).digest('hex');
    expect(recomputable(held({ docId: name }), null, bytes)).toBe(true);
  });

  it('HELD whose bytes do not hash to its name is MALFORMED, never "stale" (§2 :216-:218)', async () => {
    const { recomputable } = await predicates(['recomputable']);
    expect(recomputable(held(), null, new Uint8Array([9, 9, 9]))).toBe(false);
  });

  it('SEALED is verifiedAtReceipt being SET — an observation, never re-evaluated', async () => {
    const { recomputable } = await predicates(['recomputable']);
    expect(recomputable(sealed(), null, null)).toBe(true);
    expect(recomputable(sealed({ verifiedAtReceipt: null }), null, null)).toBe(false);
  });
  ...
  it('SEALED does NOT recompute even when bytes are handed to it — the platform cannot repeat that check', async () => {
    ...
    expect(recomputable(sealed(), null, new Uint8Array([4, 5, 6]))).toBe(true);
  });

  it('NONE answers what was LAST RECORDED, as recorded (A3 :1363)', async () => {
    const { recomputable } = await predicates(['recomputable']);
    expect(recomputable(sealed(), shedRow(), null)).toBe(true);
    expect(recomputable(held({ bytes: null }), shedRow(), null)).toBe(false);
  });
});
