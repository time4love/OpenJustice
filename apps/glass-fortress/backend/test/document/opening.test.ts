...
//
// OPENED IS THE WIDEST across every thesis a version of which EVER published cites the document (A3 :1377 as CONFORMED 2026-09-26, R84 Q2),
...
interface Openings {
  opened: (
    ...
    keyOf: (decision: DocumentOpeningDecisionRow) => string,
  ) => Opening | null;
  publicDocument: (
    commitment: string,
    decisions: readonly DocumentOpeningDecisionRow[],
...
// DECLARED EDIT, document step 34 (R84 chunk 3, REVIEW's LOW-k): the grouping key is REQUIRED — this file holds the rule
// per THESIS, so it says so; the loader groups per PUBLICATION (A4 :1444 as CONFORMED, Q14).
const byThesis = (decision: DocumentOpeningDecisionRow): string => decision.thesisId;
...
describe('A3 :1377-:1378 — OPENED(d) is the WIDEST of the decisions in force', () => {
  it('one decision on one published thesis gives that opening', async () => {
    ...
    expect(opened(COMMITMENT, [decision({ opening: 'CONTENT' })], ['th_1'], byThesis)).toBe('CONTENT');
  });
  ...
  it('two theses give the WIDER of the two — what one reader may see, every reader may', async () => {
    ...
    expect(opened(COMMITMENT, [narrow, wide], ['th_1', 'th_2'], byThesis)).toBe('BYTES');
  });

  it('a decision by a thesis that is NOT PUBLISHED does not open anything (A3 :1377)', async () => {
    const { opened } = await openings(['opened']);
    expect(opened(COMMITMENT, [decision({ opening: 'BYTES', thesisId: 'th_draft' })], [], byThesis)).toBeNull();
  });
  ...
  it('the LATEST decision per thesis is the one in force — sequence decides, not order of arrival', async () => {
    ...
    expect(opened(COMMITMENT, [later, first], ['th_1'], byThesis)).toBe('CONTENT');
  });

  it('none decided is NULL — and null is "not public", never a default of PASSAGE', async () => {
    const { opened } = await openings(['opened']);
    expect(opened(COMMITMENT, [], ['th_1'], byThesis)).toBeNull();
  });
});
...
describe('A3 :1379-:1380 — PUBLIC(d) is PER DOCUMENT, and there is no PUBLIC_PAGE analogue', () => {
  it('public exactly when OPENED is defined', async () => {
    ...
    expect(publicDocument(COMMITMENT, [decision()], ['th_1'], byThesis)).toBe(true);
    expect(publicDocument(COMMITMENT, [], ['th_1'], byThesis)).toBe(false);
  });

  it('ANOTHER document of the same arrival is NOT opened by this one (§7 :859-:860)', async () => {
    const { publicDocument } = await openings(['publicDocument']);
    const sibling = '0x' + 'c2'.repeat(32);
    expect(publicDocument(sibling, [decision({ commitment: COMMITMENT })], ['th_1'], byThesis)).toBe(false);
  });
});
